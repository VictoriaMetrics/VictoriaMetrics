...
for building horizontally scalable cluster. 
...
compare it to other process, perform some calculations with it or even define events to trigger on reaching 
...
Let's start with example. To track how many requests our application serves,
we'll define a metric with name `requests_total`.
...
The concept of a metric type exists specifically to help user to understand how the metric was measured.
...
process allocates and frees the memory. 
...
using the histogram with name `vm_per_query_rows_processed_count`.
...
Prometheus histogram requires user to define ranges (`buckets`) statically.
...
histogram automatically adjusts buckets, so user don't need to think about them.
...
But taking into a account the limitation mentioned above.


#### Instrumenting application with metrics

As was said in the beginning of the section [Types of metrics](#types-of-metrics), metric type defines
...
on application which emits them.
...
We recommend choosing from already existing clients compatible with listed above protocols
...
* with batch size of which size, at which rate;
* with which retry logic;
* simpler security management, the only access needed for application is the access to the TSDB.
...
* using monitoring system it is hard to tell whether application
...
The pros of pull model:

* monitoring system decides how and when to scrape data,
so it can't be overloaded;
* applications aren't aware of monitoring system and don't need
to implement logic for delivering metrics;
* the list of all monitored targets belongs to monitoring system
and can be quickly checked;
* easy to detect faulty or crashed services when they don't respond.

The cons of pull model:

* monitoring system needs access to applications it monitors;
* the frequency at which metrics are collected depends on monitoring system.

### Common approaches for data collection

VictoriaMetrics supports both [Push](#push-model) and [Pull](#pull-model)
models for data collection. There are many installations which are using
