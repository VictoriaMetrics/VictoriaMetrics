...
* Can aggregate incoming samples by time and by labels before sending them to remote storage - see [these docs](./stream-aggregation.md).
...
* Can deal with [high cardinality](./FAQ.md#what-is-high-cardinality)
  and [high churn rate](./FAQ.md#what-is-high-churn-rate) issues by limiting the number of unique time series at scrape time
...
* `-promscrape.config` with the path to [Prometheus config file](./sd_configs.md) (usually located at `/etc/prometheus/prometheus.yml`).
  The path can point either to local file or to http url. See [scrape config examples](./scrape_config_examples.md).
...
to [single-node VictoriaMetrics](./Single-server-VictoriaMetrics.md) located at `victoria-metrics-host:8428`:

```sh
/path/to/vmagent -remoteWrite.url=https://victoria-metrics-host:8428/api/v1/write
```

See [these docs](./Cluster-VictoriaMetrics.md#url-format) if you need writing
the data to [VictoriaMetrics cluster](./Cluster-VictoriaMetrics.md).
...
without the need to use `vmagent` - see [these docs](./#how-to-scrape-prometheus-exporters-such-as-node-exporter).
...
`vmagent` supports [the same set of push-based data ingestion protocols as VictoriaMetrics does](./#how-to-import-time-series-data)
additionally to pull-based Prometheus-compatible targets' scraping:

* DataDog "submit metrics" API. See [these docs](./Single-Server-VictoriaMetrics.md#how-to-send-data-from-datadog-agent).
* InfluxDB line protocol via `http://<vmagent>:8429/write`. See [these docs](./Single-Server-VictoriaMetrics.md#how-to-send-data-from-influxdb-compatible-agents-such-as-telegraf).
* Graphite plaintext protocol if `-graphiteListenAddr` command-line flag is set. See [these docs](./Single-Server-VictoriaMetrics.md#how-to-send-data-from-graphite-compatible-agents-such-as-statsd).
* OpenTelemetry http API. See [these docs](./Single-Server-VictoriaMetrics.md#sending-data-via-opentelemetry).
* NewRelic API. See [these docs](./Single-Server-VictoriaMetrics.md#how-to-send-data-from-newrelic-agent).
* OpenTSDB telnet and http protocols if `-opentsdbListenAddr` command-line flag is set. See [these docs](./Single-Server-VictoriaMetrics.md#how-to-send-data-from-opentsdb-compatible-agents).
* Prometheus remote write protocol via `http://<vmagent>:8429/api/v1/write`.
* JSON lines import protocol via `http://<vmagent>:8429/api/v1/import`. See [these docs](./Single-Server-VictoriaMetrics.md#how-to-import-data-in-json-line-format).
* Native data import protocol via `http://<vmagent>:8429/api/v1/import/native`. See [these docs](./Single-Server-VictoriaMetrics.md#how-to-import-data-in-native-format).
* Prometheus exposition format via `http://<vmagent>:8429/api/v1/import/prometheus`. See [these docs](./Single-Server-VictoriaMetrics.md#how-to-import-data-in-prometheus-exposition-format) for details.
* Arbitrary CSV data via `http://<vmagent>:8429/api/v1/import/csv`. See [these docs](./Single-Server-VictoriaMetrics.md#how-to-import-csv-data).
...
when [stream aggregation](./stream-aggregation.md) is enabled.
See [these docs](./stream-aggregation.md#statsd-alternative) for details.
...
[VictoriaMetrics cluster](./Cluster-VictoriaMetrics.md) already supports replication,
so there is no need in specifying multiple `-remoteWrite.url` flags when writing data to the same cluster.
See [these docs](./Cluster-VictoriaMetrics.md#replication-and-data-safety).
...
the outgoing [time series](./keyConcepts.md#time-series) among all the remote storage systems
...
For example, this allows building horizontally scalable [stream aggregation](./stream-aggregation.md)
by routing outgoing samples for the same time series of [counter](./keyConcepts.md#counter)
and [histogram](./keyConcepts.md#histogram) types from top-level `vmagent` instances
...
For example, if all the [raw samples](./keyConcepts.md#raw-samples) with the same set of labels
...
writes are always performed in Prometheus remote_write protocol. Therefore, for the [clustered version](./Cluster-VictoriaMetrics.md),
...
[Deduplication at stream aggregation](./stream-aggregation.md#deduplication) allows setting up arbitrary complex de-duplication schemes
for the collected samples. Examples:

- The following command instructs `vmagent` to send only the last sample per each seen [time series](./keyConcepts.md#time-series) per every 60 seconds:
  ```
  ./vmagent -remoteWrite.url=http://remote-storage/api/v1/write -remoteWrite.streamAggr.dedupInterval=60s
  ```

- The following command instructs `vmagent` to merge [time series](./keyConcepts.md#time-series) with different `replica` label values
...
- In scrape target addresses aka `__address__` label - see [these docs](./relabeling.md#how-to-modify-scrape-urls-in-targets) for details.

- In urls used for [service discovery](./sd_configs.md).
...
or `vminsert` at [cluster version](./Cluster-VictoriaMetrics.md).
...
By default `vmagent` collects the data without [tenant](./Cluster-VictoriaMetrics.md#multitenancy) identifiers
...
at `vminsert` according to [these docs](./Cluster-VictoriaMetrics.md#url-format). In this case all the metrics are written to the given `<tenant_id>` tenant.
...
and then to push metrics with these labels to [multitenant url at VictoriaMetrics cluster](./Cluster-VictoriaMetrics.md#multitenancy-via-labels).
...
For example, the following relabeling rule instructs sending metrics to `<account_id>:0` [tenant](./Cluster-VictoriaMetrics.md#multitenancy)
...
`vmagent` can accept data via the same multitenant endpoints (`/insert/<accountID>/<suffix>`) as `vminsert` at [VictoriaMetrics cluster](./Cluster-VictoriaMetrics.md)
does according to [these docs](./Cluster-VictoriaMetrics.md#url-format) if `-enableMultitenantHandlers` command-line flag is set.
...
when specifying `-remoteWrite.url` to [multitenant url at VictoriaMetrics cluster](./Cluster-VictoriaMetrics.md#multitenancy-via-labels).
...
`vmagent` supports the following additional options in [scrape_configs](./sd_configs.md#scrape_configs) section:
...
See [scrape_configs docs](./sd_configs.md#scrape_configs) for more details on all the supported options.


## Loading scrape configs from multiple files

`vmagent` supports loading [scrape configs](./sd_configs.md#scrape_configs) from multiple files specified
...
Every referred file can contain arbitrary number of [supported scrape configs](./sd_configs.md#scrape_configs).
...
* `remote_read`. This section isn't supported at all, since `vmagent` doesn't provide Prometheus querying API.
  ...
  See [Prometheus querying API docs for VictoriaMetrics](./#prometheus-querying-api-usage).
* `rule_files` and `alerting`. These sections are supported by [vmalert](./vmalert.md).
...
* `up` - this metric exposes `1` value on successful scrape and `0` value on unsuccessful scrape. This allows monitoring
  failing scrapes with the following [MetricsQL query](./MetricsQL.md):
...
* `scrape_duration_seconds` - the duration of the scrape for the given target. This allows monitoring slow scrapes.
  For example, the following [MetricsQL query](./MetricsQL.md) returns scrapes,
...
* `scrape_timeout_seconds` - the configured timeout for the current scrape target (aka `scrape_timeout`).
  ...
  For example, the following [MetricsQL query](./MetricsQL.md) returns targets (identified by `instance` label),
...
* `scrape_response_size_bytes` - response size in bytes for the given target. This allows to monitor amount of data scraped
  and to adjust [`max_scrape_size` option](./sd_configs.md#scrape_configs) for scraped targets.
  For example, the following [MetricsQL query](./MetricsQL.md) returns targets with scrape response
  bigger than `10MiB`:
...
* `scrape_samples_scraped` - the number of samples (aka metrics) parsed per each scrape. This allows detecting targets,
  which expose too many metrics. For example, the following [MetricsQL query](./MetricsQL.md)
...
* `scrape_samples_limit` - the configured limit on the number of metrics the given target can expose.
  The limit can be set via `sample_limit` option at [scrape_configs](./sd_configs.md#scrape_configs).
...
* `scrape_samples_post_metric_relabeling` - the number of samples (aka metrics) left after applying metric-level relabeling
  ...
  For example, the following [MetricsQL query](./MetricsQL.md) returns targets
...
* `scrape_series_added` - **an approximate** number of new series the given target generates during the current scrape.
  ...
  which lead to [high churn rate](./FAQ.md#what-is-high-churn-rate).
  For example, the following [MetricsQL query](./MetricsQL.md) returns targets,
...
* At the `scrape_config -> relabel_configs` section in `-promscrape.config` file.
  ...
  See [relabeling cookbook](./relabeling.md) for details.
...
[Streaming aggregation](./stream-aggregation.md), if configured,
...
* An optional `if` filter can be used for conditional relabeling. The `if` filter may contain
  arbitrary [time series selector](./keyConcepts.md#filtering).
  The `action` is performed only for [samples](./keyConcepts.md#raw-samples), which match the provided `if` filter.
  ...
  matches the given [sample](./keyConcepts.md#raw-samples).
...
`vmagent` and [single-node VictoriaMetrics](./#how-to-scrape-prometheus-exporters-such-as-node-exporter)
provide the following tools for debugging target-level and metric-level relabeling:

- Target-level debugging (e.g. `relabel_configs` section at [scrape_configs](./sd_configs.md#scrape_configs))
...
- Metric-level debugging (e.g. `metric_relabel_configs` section at [scrape_configs](./sd_configs.md#scrape_configs)
  can be performed by navigating to `http://vmagent:8429/targets` page (`http://victoriametrics:8428/targets` page for single-node VictoriaMetrics)
...
- `http://vmagent:8429/service-discovery` page, which contains information about all the [discovered targets](./sd_configs.md).
...
* By specifying `no_stale_markers: true` option in the [scrape_config](./sd_configs.md#scrape_configs) for the corresponding target.
...
See [these docs](./#deduplication) for details.
...
When running HA pairs, [deduplication](./#deduplication) must be configured 
...
* `proxy_authorization` for generic token authorization. See [these docs](./sd_configs.md#http-api-client-options).
* `proxy_basic_auth` for Basic authorization. See [these docs](./sd_configs.md#http-api-client-options).
...
- It drops [stream aggregation](./stream-aggregation.md) output samples, because it is better from operations perspective
...
if it cannot keep up with the data ingestion rate. In this case the [deduplication](./#deduplication)
...
* Via `series_limit` config option at [scrape_config](./sd_configs.md#scrape_configs) section.
...
See also `sample_limit` option at [scrape_config section](./sd_configs.md#scrape_configs).
...
See also [cardinality explorer docs](./#cardinality-explorer).
...
* `http://vmagent-host:8429/service-discovery`. This pages shows the list of discovered targets with the discovered `__meta_*` labels
  according to [these docs](./sd_configs.md).
...
* `http://vmagent-host:8429/ready`. This handler returns http 200 status code when `vmagent` finishes
  its initialization for all the [service_discovery configs](./sd_configs.md).
...
* If `vmagent` uses too big amounts of memory, then the following options can help:
  ...
  * Passing `-promscrape.dropOriginalLabels` command-line flag to `vmagent` if it [discovers](./sd_configs.md)
    big number of targets and many of these targets are [dropped](./relabeling.md#how-to-drop-discovered-targets)
    ...
    of reduced debuggability for improperly configured per-target [relabeling](./relabeling.md).
...
* Use `-remoteWrite.queues=1` when `-remoteWrite.url` points to remote storage, which doesn't accept out-of-order samples (aka data backfilling).
  ...
  The best solution is to use remote storage with [backfilling support](./#backfilling) such as VictoriaMetrics.
...
1. The **size in bytes** of data stream sent by vmagent:
   ...
   Run the query `sum(rate(vmagent_remotewrite_bytes_sent_total[1h])) by(instance,url)` in [vmui](./#vmui)
...
1. Re-evaluate the estimation each time when:
    ...
    * there is a change in [relabeling rules](./vmagent.md#relabeling) which could increase the amount metrics to send
...
1. On-disk persistent queue can be disabled if needed. See [these docs](./vmagent.md#disabling-on-disk-persistence).


## Google PubSub integration

[Enterprise version](./enterprise.md) of `vmagent` can read and write metrics from / to [Google PubSub](https://cloud.google.com/pubsub):

### Reading metrics from PubSub

[Enterprise version](./enterprise.md) of `vmagent` can read metrics in various formats from Google PubSub messages.
...
* `jsonline` - [JSON line format](./#how-to-import-data-in-json-line-format).
...
See [these docs](./vmagent.md#disabling-on-disk-persistence).

See also [how to write metrics to multiple distinct tenants](./vmagent.md#multitenancy).
...
and gzipped [JSON line](./#json-line-format) messages from `json-line-testing` topic:
...
These command-line flags are available only in [enterprise](./enterprise.md) version of `vmagent`,
...
```sh
  -gcp.pubsub.subscribe.credentialsFile string
        Path to file with GCP credentials to use for PubSub client. If not set, default credentials are used (see Workload Identity for K8S or https://cloud.google.com/docs/authentication/application-default-credentials ). See {{% ref "./vmagent.md#reading-metrics-from-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
  -gcp.pubsub.subscribe.defaultMessageFormat string
        Default message format if -gcp.pubsub.subscribe.topicSubscription.messageFormat is missing. See {{% ref "./vmagent.md#reading-metrics-from-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}} (default "promremotewrite")
  -gcp.pubsub.subscribe.topicSubscription array
        GCP PubSub topic subscription in the format: projects/<project-id>/subscriptions/<subscription-name>. See {{% ref "./vmagent.md#reading-metrics-from-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports an array of values separated by comma or specified via multiple flags.
  -gcp.pubsub.subscribe.topicSubscription.concurrency array
        The number of concurrently processed messages for topic subscription specified via -gcp.pubsub.subscribe.topicSubscription flag. See {{% ref "./vmagent.md#reading-metrics-from-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}} (default 0)
        Supports array of values separated by comma or specified via multiple flags.
  -gcp.pubsub.subscribe.topicSubscription.isGzipped array
        Enables gzip decompression for messages payload at the corresponding -gcp.pubsub.subscribe.topicSubscription. Only prometheus, jsonline, graphite and influx formats accept gzipped messages. See {{% ref "./vmagent.md#reading-metrics-from-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports array of values separated by comma or specified via multiple flags.
  -gcp.pubsub.subscribe.topicSubscription.messageFormat array
        Message format for the corresponding -gcp.pubsub.subscribe.topicSubscription. Valid formats: influx, prometheus, promremotewrite, graphite, jsonline . See {{% ref "./vmagent.md#reading-metrics-from-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports an array of values separated by comma or specified via multiple flags.
```

### Writing metrics to PubSub

[Enterprise version](./enterprise.md) of `vmagent` writes data into Google PubSub if `-remoteWrite.url` command-line flag starts with `pubsub:` prefix.
...
```sh
  -gcp.pubsub.publish.byteThreshold int
        Publish a batch when its size in bytes reaches this value. See {{% ref "./vmagent.md#writing-metrics-to-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}} (default 1000000)
  -gcp.pubsub.publish.countThreshold int
        Publish a batch when it has this many messages. See {{% ref "./vmagent.md#writing-metrics-to-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}} (default 100)
  -gcp.pubsub.publish.credentialsFile string
        Path to file with GCP credentials to use for PubSub client. If not set, default credentials will be used (see Workload Identity for K8S or https://cloud.google.com/docs/authentication/application-default-credentials). See {{% ref "./vmagent.md#writing-metrics-to-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
  -gcp.pubsub.publish.delayThreshold value
        Publish a non-empty batch after this delay has passed. See {{% ref "./vmagent.md#writing-metrics-to-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
  ...
  -gcp.pubsub.publish.maxOutstandingBytes int
        The maximum size of buffered messages to be published. If less than or equal to zero, this is disabled. See {{% ref "./vmagent.md#writing-metrics-to-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}} (default -1)
  -gcp.pubsub.publish.maxOutstandingMessages int
        The maximum number of buffered messages to be published. If less than or equal to zero, this is disabled. See {{% ref "./vmagent.md#writing-metrics-to-pubsub" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}} (default 100)
  -gcp.pubsub.publish.timeout value
        The maximum time that the client will attempt to publish a bundle of messages. See {{% ref "./vmagent.md#writing-metrics-to-pubsub" %}} . This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
...
[Enterprise version](./enterprise.md) of `vmagent` can read and write metrics from / to Kafka:
...
[Enterprise version](./enterprise.md) of `vmagent` can read metrics in various formats from Kafka messages.
...
or [the VictoriaMetrics remote write protocol](./vmagent.md#victoriametrics-remote-write-protocol), and handle them accordingly.
...
Note that when using YAML configuration (for example, when using [Helm charts](https://github.com/VictoriaMetrics/helm-charts) or [Kubernetes operator](./operator/README.md))
...
```sh
  -kafka.consumer.topic array
        Kafka topic names for data consumption. See {{% ref "./vmagent.md#reading-metrics-from-kafka" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports an array of values separated by comma or specified via multiple flags.
  -kafka.consumer.topic.basicAuth.password array
        Optional basic auth password for -kafka.consumer.topic.  Must be used in conjunction with any supported auth methods for kafka client, specified by flag -kafka.consumer.topic.options='security.protocol=SASL_SSL;sasl.mechanisms=PLAIN' . See {{% ref "./vmagent.md#reading-metrics-from-kafka" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports an array of values separated by comma or specified via multiple flags.
  -kafka.consumer.topic.basicAuth.username array
        Optional basic auth username for -kafka.consumer.topic. Must be used in conjunction with any supported auth methods for kafka client, specified by flag -kafka.consumer.topic.options='security.protocol=SASL_SSL;sasl.mechanisms=PLAIN' . See {{% ref "./vmagent.md#reading-metrics-from-kafka" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports an array of values separated by comma or specified via multiple flags.
  -kafka.consumer.topic.brokers array
        List of brokers to connect for given topic, e.g. -kafka.consumer.topic.broker=host-1:9092;host-2:9092 . See {{% ref "./vmagent.md#reading-metrics-from-kafka" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports an array of values separated by comma or specified via multiple flags.
  -kafka.consumer.topic.concurrency array
        Configures consumer concurrency for topic specified via -kafka.consumer.topic flag. See {{% ref "./vmagent.md#reading-metrics-from-kafka" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}} (default 1)
        Supports array of values separated by comma or specified via multiple flags.
  -kafka.consumer.topic.defaultFormat string
        Expected data format in the topic if -kafka.consumer.topic.format is skipped. See {{% ref "./vmagent.md#reading-metrics-from-kafka" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}} (default "promremotewrite")
  -kafka.consumer.topic.format array
        data format for corresponding kafka topic. Valid formats: influx, prometheus, promremotewrite, graphite, jsonline . See {{% ref "./vmagent.md#reading-metrics-from-kafka" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports an array of values separated by comma or specified via multiple flags.
  -kafka.consumer.topic.groupID array
        Defines group.id for topic. See {{% ref "./vmagent.md#reading-metrics-from-kafka" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports an array of values separated by comma or specified via multiple flags.
  -kafka.consumer.topic.isGzipped array
        Enables gzip setting for topic messages payload. Only prometheus, jsonline, graphite and influx formats accept gzipped messages.See {{% ref "./vmagent.md#reading-metrics-from-kafka" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports array of values separated by comma or specified via multiple flags.
  -kafka.consumer.topic.options array
        Optional key=value;key1=value2 settings for topic consumer. See full configuration options at https://github.com/edenhill/librdkafka/blob/master/CONFIGURATION.md . See {{% ref "./vmagent.md#reading-metrics-from-kafka" %}}. This flag is available only in Enterprise binaries. See {{% ref "./enterprise.md" %}}
        Supports an array of values separated by comma or specified via multiple flags.
```

### Writing metrics to Kafka

[Enterprise version](./enterprise.md) of `vmagent` writes data to Kafka with `at-least-once`
...
To switch to [the VictoriaMetrics remote write protocol](./vmagent.md#victoriametrics-remote-write-protocol) and reduce network bandwidth,
...
See the docs at {{% ref "./vmagent.md" %}}.
  ...
  -enableMultitenantHandlers
     Whether to process incoming data via multitenant insert handlers according to {{% ref "./Cluster-VictoriaMetrics.md#url-format" %}}. By default incoming data is processed via single-node insert handlers according to {{% ref "./#how-to-import-time-series-data" %}}.See {{% ref "./vmagent.md#multitenancy" %}} for details
  -enableTCP6
  ...
  -promscrape.config string
     Optional path to Prometheus config file with 'scrape_configs' section containing targets to scrape. The path can point to local file and to http url. See {{% ref "./#how-to-scrape-prometheus-exporters-such-as-node-exporter" %}} for details
  ...
  -promscrape.vultrSDCheckInterval duration
     Interval for checking for changes in Vultr. This works only if vultr_sd_configs is configured in '-promscrape.config' file. See {{% ref "./sd_configs.md#vultr_sd_configs" %}} for details  (default 30s)
  ...
  -remoteWrite.disableOnDiskQueue array
     Whether to disable storing pending data to -remoteWrite.tmpDataPath when the remote storage system at the corresponding -remoteWrite.url cannot keep up with the data ingestion rate. See {{% ref "./vmagent.md#disabling-on-disk-persistence" %}}. See also -remoteWrite.dropSamplesOnOverload
     Supports array of values separated by comma or specified via multiple flags.
     Empty values are set to false.
  -remoteWrite.dropSamplesOnOverload
     Whether to drop samples when -remoteWrite.disableOnDiskQueue is set and if the samples cannot be pushed into the configured -remoteWrite.url systems in a timely manner. See {{% ref "./vmagent.md#disabling-on-disk-persistence" %}}
  -remoteWrite.flushInterval duration
     Interval for flushing the data to remote storage. This option takes effect only when less than 10K data points per second are pushed to -remoteWrite.url (default 1s)
  -remoteWrite.forcePromProto array
     Whether to force Prometheus remote write protocol for sending data to the corresponding -remoteWrite.url . See {{% ref "./vmagent.md#victoriametrics-remote-write-protocol" %}}
     Supports array of values separated by comma or specified via multiple flags.
     Empty values are set to false.
  -remoteWrite.forceVMProto array
     Whether to force VictoriaMetrics remote write protocol for sending data to the corresponding -remoteWrite.url . See {{% ref "./vmagent.md#victoriametrics-remote-write-protocol" %}}
     Supports array of values separated by comma or specified via multiple flags.
     Empty values are set to false.
  ...
  -remoteWrite.maxDailySeries int
     The maximum number of unique series vmagent can send to remote storage systems during the last 24 hours. Excess series are logged and dropped. This can be useful for limiting series churn rate. See {{% ref "./vmagent.md#cardinality-limiter" %}}
  ...
  -remoteWrite.maxHourlySeries int
     The maximum number of unique series vmagent can send to remote storage systems during the last hour. Excess series are logged and dropped. This can be useful for limiting series cardinality. See {{% ref "./vmagent.md#cardinality-limiter" %}}
  ...
  -remoteWrite.relabelConfig string
     Optional path to file with relabeling configs, which are applied to all the metrics before sending them to -remoteWrite.url. See also -remoteWrite.urlRelabelConfig. The path can point either to local file or to http url. See {{% ref "./vmagent.md#relabeling" %}}
  ...
  -remoteWrite.shardByURL
     Whether to shard outgoing series across all the remote storage systems enumerated via -remoteWrite.url . By default the data is replicated across all the -remoteWrite.url . See {{% ref "./vmagent.md#sharding-among-remote-storages" %}}. See also -remoteWrite.shardByURLReplicas
  ...
  -remoteWrite.shardByURLReplicas int
     How many copies of data to make among remote storage systems enumerated via -remoteWrite.url when -remoteWrite.shardByURL is set. See {{% ref "./vmagent.md#sharding-among-remote-storages" %}} (default 1)
  -remoteWrite.showURL
  ...
  -remoteWrite.streamAggr.config array
     Optional path to file with stream aggregation config for the corresponding -remoteWrite.url. See {{% ref "./stream-aggregation.md" %}}. See also -remoteWrite.streamAggr.keepInput, -remoteWrite.streamAggr.dropInput and -remoteWrite.streamAggr.dedupInterval
     Supports an array of values separated by comma or specified via multiple flags.
     Value can contain comma inside single-quoted or double-quoted string, {}, [] and () braces.
  -remoteWrite.streamAggr.dedupInterval array
     Input samples are de-duplicated with this interval before optional aggregation with -remoteWrite.streamAggr.config at the corresponding -remoteWrite.url. See also -dedup.minScrapeInterval and {{% ref "./stream-aggregation.md#deduplication" %}} (default 0s)
     Supports array of values separated by comma or specified via multiple flags.
     Empty values are set to default value.
  -remoteWrite.streamAggr.dropInput array
     Whether to drop all the input samples after the aggregation with -remoteWrite.streamAggr.config at the corresponding -remoteWrite.url. By default, only aggregates samples are dropped, while the remaining samples are written to the corresponding -remoteWrite.url . See also -remoteWrite.streamAggr.keepInput and {{% ref "./stream-aggregation.md" %}}
     Supports array of values separated by comma or specified via multiple flags.
     Empty values are set to false.
  -remoteWrite.streamAggr.dropInputLabels array
     An optional list of labels to drop from samples before stream de-duplication and aggregation with -remoteWrite.streamAggr.config and -remoteWrite.streamAggr.dedupInterval at the corresponding -remoteWrite.url. See {{% ref "./stream-aggregation.md#dropping-unneeded-labels" %}}
     Supports an array of values separated by comma or specified via multiple flags.
     Value can contain comma inside single-quoted or double-quoted string, {}, [] and () braces.
  -remoteWrite.streamAggr.ignoreFirstIntervals int
     Number of aggregation intervals to skip after the start for the corresponding -remoteWrite.streamAggr.config at the corresponding -remoteWrite.url. Increase this value if you observe incorrect aggregation results after vmagent restarts. It could be caused by receiving bufferred delayed data from clients pushing data into the vmagent. See {{% ref "./stream-aggregation.md#ignore-aggregation-intervals-on-start" %}}
  -remoteWrite.streamAggr.ignoreOldSamples array
     Whether to ignore input samples with old timestamps outside the current aggregation interval for the corresponding -remoteWrite.streamAggr.config at the corresponding -remoteWrite.url. See {{% ref "./stream-aggregation.md#ignoring-old-samples" %}}
     Supports array of values separated by comma or specified via multiple flags.
     Empty values are set to false.
  -remoteWrite.streamAggr.keepInput array
     Whether to keep all the input samples after the aggregation with -remoteWrite.streamAggr.config at the corresponding -remoteWrite.url. By default, only aggregates samples are dropped, while the remaining samples are written to the corresponding -remoteWrite.url . See also -remoteWrite.streamAggr.dropInput and {{% ref "./stream-aggregation.md" %}}
     Supports array of values separated by comma or specified via multiple flags.
     Empty values are set to false.
  ...
  -remoteWrite.urlRelabelConfig array
     Optional path to relabel configs for the corresponding -remoteWrite.url. See also -remoteWrite.relabelConfig. The path can point either to local file or to http url. See {{% ref "./vmagent.md#relabeling" %}}
     Supports an array of values separated by comma or specified via multiple flags.
     Value can contain comma inside single-quoted or double-quoted string, {}, [] and () braces.
  -remoteWrite.vmProtoCompressLevel int
     The compression level for VictoriaMetrics remote write protocol. Higher values reduce network traffic at the cost of higher CPU usage. Negative values reduce CPU usage at the cost of increased network traffic. See {{% ref "./vmagent.md#victoriametrics-remote-write-protocol" %}}
  -sortLabels
  ...
  -streamAggr.dedupInterval value
    Input samples are de-duplicated with this interval on aggregator before optional aggregation with -streamAggr.config . See also -dedup.minScrapeInterval and {{% ref "./stream-aggregation.md#deduplication" %}}
  ...
  -streamAggr.dropInput
    Whether to drop all the input samples after the aggregation with -remoteWrite.streamAggr.config. By default, only aggregates samples are dropped, while the remaining samples are written to remote storages write. See also -streamAggr.keepInput and {{% ref "./stream-aggregation.md" %}}
  -streamAggr.dropInputLabels array
    An optional list of labels to drop from samples for aggregator before stream de-duplication and aggregation . See {{% ref "./stream-aggregation.md#dropping-unneeded-labels" %}}
    Supports an array of values separated by comma or specified via multiple flags.
    Value can contain comma inside single-quoted or double-quoted string, {}, [] and () braces.
  -streamAggr.ignoreFirstIntervals int
    Number of aggregation intervals to skip after the start for aggregator. Increase this value if you observe incorrect aggregation results after vmagent restarts. It could be caused by receiving unordered delayed data from clients pushing data into the vmagent. See {{% ref "./stream-aggregation.md#ignore-aggregation-intervals-on-start" %}}
  -streamAggr.ignoreOldSamples
    Whether to ignore input samples with old timestamps outside the current aggregation interval for aggregator. See {{% ref "./stream-aggregation.md#ignoring-old-samples" %}}
  -streamAggr.keepInput
    Whether to keep all the input samples after the aggregation with -streamAggr.config. By default, only aggregates samples are dropped, while the remaining samples are written to remote storages write. See also -streamAggr.dropInput and {{% ref "./stream-aggregation.md" %}}
  -tls array
