...
- By splitting **queries** [defined in the reader section](https://docs.victoriametrics.com/anomaly-detection/components/reader#vm-reader) and assigning each subset to a separate service instance. If you have *a single query returning a large number of timeseries*, you can further split it by applying global MetricsQL filters using the `extra_filters` [parameter in the reader](https://docs.victoriametrics.com/anomaly-detection/components/reader?highlight=extra_filters#vm-reader). See the example below.

- By splitting **models** (e.g., if you decide to run multiple models per timeseries for use cases such as averaging anomaly scores in your alerting rules with `vmalert`, or employing a voting approach to reduce false positives). See the `queries` argument in the [model configuration](https://docs.victoriametrics.com/anomaly-detection/components/models#queries). Additionally, this approach is useful when you just have a large set of resource-intensive independent models.

- By splitting **schedulers** (e.g., if you need the same models to be trained or inferred under different schedules). Refer to the `schedulers` argument in the [model section](https://docs.victoriametrics.com/anomaly-detection/components/models#schedulers) and the `scheduler` [component documentation](https://docs.victoriametrics.com/anomaly-detection/components/scheduler).

### Splitting the config

Starting from [v1.18.5](https://docs.victoriametrics.com/anomaly-detection/changelog/#v1185), a CLI utility `config_splitter.py` is available. This tool enables splitting a parent YAML configuration file into multiple sub-configurations based on logical entities (`schedulers`, `queries`, `models`, `extra_filters`). The resulting sub-configurations are fully validated and functional, accounting for many-to-many relationships between models, their associated queries, and the schedulers they are linked to. These sub-configurations can then be saved to a specified directory for further use:
