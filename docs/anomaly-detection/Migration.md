...
|---------|--------- |------------|-------|
| [v1.29.1](https://docs.victoriametrics.com/anomaly-detection/changelog/#v1291) | [v1.30.3](https://docs.victoriametrics.com/anomaly-detection/changelog/#v1303) | Fully Compatible* | v1.30.0 adds new [Temporal Envelope](https://docs.victoriametrics.com/anomaly-detection/components/models/#temporal-envelope) model state without changing the compatibility of existing built-in model and data artifacts. v1.30.3 remains compatible with v1.30.2 built-in model and reader/data state and its compatible predecessors; no persisted-state migration is required for those components. The asterisk excludes the custom Python class contract: models that relied only on `is_multivariate = True` must declare `topology = ModelTopology.MANY_TO_ONE` before upgrading. Because topology selects service routing and persisted model identity, prior custom many-to-one state is not guaranteed to be reused until the class adopts the explicit [model topology contract](https://docs.victoriametrics.com/anomaly-detection/components/models/#custom-model-guide). |
