...
```sh
  ...
  -streamAggr.dropInputLabels array
     ...
     Supports an array of values separated by comma or specified via multiple flags.
     Value can contain comma inside single-quoted or double-quoted string, {}, [] and () braces.
  -streamAggr.ignoreOldIntervals
     Whether to ignore input intervals that contain samples with old timestamps outside the current aggregation interval. See https://docs.victoriametrics.com/stream-aggregation.html#ignoring-old-intervals
