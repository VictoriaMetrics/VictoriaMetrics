...
* BUGFIX: fix VictoriaLogs startup failure when `parts.json` file is missing from datadb directory. VictoriaLogs now automatically scans for existing parts and recreates the `parts.json` file, preventing crashes during startup. This commonly occurs after unclean shutdowns or storage corruption. See [#8873](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/8873).
