...
Field `interval` is recommended to be set to the value that are at least several times higher than your scrape/push interval.
...
If the monitored app calculates some events and then sends the calculated number of events to VictoriaMetrics
