...
1. Multi-level [vmselect setup](https://docs.victoriametrics.com/victoriametrics/cluster-version/#multi-level-cluster-setup) in cluster setup, top-level vmselect(s) reads data from cluster-level vmselects
   * Returns data in one of the clusters is unavailable  
   * Merges data from both sources. You need to turn on [deduplication](https://docs.victoriametrics.com/victoriametrics/cluster-version/#deduplication) to remove duplicates 
...
1. Global vmselect in cluster setup - you can set up an additional subset of vmselects that knows about all storages in all regions.
   * The [deduplication](https://docs.victoriametrics.com/victoriametrics/cluster-version/#deduplication) in 1ms on the vmselect side must be turned on. This setup allows you to query data using MetricsQL.
...
Additional context
* VictoriaMetrics Single - [https://docs.victoriametrics.com/victoriametrics/single-node-version/#monitoring](https://docs.victoriametrics.com/victoriametrics/single-node-version/#monitoring)
* VictoriaMetrics Cluster - [https://docs.victoriametrics.com/victoriametrics/cluster-version/#monitoring](https://docs.victoriametrics.com/victoriametrics/cluster-version/#monitoring)
