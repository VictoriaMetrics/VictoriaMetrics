...
* BUGFIX: `vmstorage`: properly handle `extra_filters` and `extra_label` for `/labels` and `/label/{}/values` APIs. Previously it may return an error with `-search.max` limit exceeds. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/5055) for details.
