...
**Update note 3: the `maxLabelValueLen` command-line flag default value was changed from 16kB to 1kB. It may trigger new timeseries registration after upgrade and truncate labels with enormous values.**
...
* BUGFIX: [Single-node VictoriaMetrics](https://docs.victoriametrics.com/) and `vminsert` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/cluster-victoriametrics/): change default value for `-maxLabelValueLen` flag to `1024`. It must fix possible issue with indexDB items drop and reduce memory usage for metrics with oversized label values. See this [issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/6176) for details.
