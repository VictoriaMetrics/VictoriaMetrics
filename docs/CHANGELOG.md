...
* FEATURE: [vmalert](https://docs.victoriametrics.com/vmalert/): adds `datasource.idleConnTimeout`, `remoteWrite.idleConnTimeout` and `remoteRead.idleConnTimeout` flags. Which must reduce probability of `broken pipe` and `connection reset by peer` errors. See this [issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/5661) for details.
* FEATURE: [vmalert](https://docs.victoriametrics.com/vmalert/): Adds request retry for trivial network errors, such as `broken pipe` and `connection reset`. See this [issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/5661) for details.
