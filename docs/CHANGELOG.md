...
* FEATURE: `vmstorage`: add `-storage.gracefulShutdownDuration` command-line flag for gradual connections termination between vminserts and vmstorage. This should reduce the impact generated by re-routing data samples from terminated vmstorage node to the remaining nodes. Please note, the `-storage.gracefulShutdownDuration` has a default value to enable graceful shutdown of vmstorage nodes. To disable this behavior, set `-storage.gracefulShutdownDuration=0s`. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/4922) and [these docs](https://docs.victoriametrics.com/Cluster-VictoriaMetrics.html#improving-re-routing-performance-during-restart) for details.
