...
* FEATURE: `vmstorage`: add `-storage.gracefulShutdownDuration` command-line flag which can be used for enhancing re-routing performance during `vmstorage` restarts. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/4922) and [these docs](https://docs.victoriametrics.com/Cluster-VictoriaMetrics.html#improving-re-routing-performance-during-restart) for details.
