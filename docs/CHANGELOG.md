...
* BUGFIX: `vmselect`: check if deadline configured via `-search.maxQueryDuration` command-line flag is exceeded during processing of the data blocks received from vmstorage. Before, the deadline could have been exceeded when persisting temporary data blocks on the disk took significant amount of time. 
