...
* FEATURE: [vmagent](https://docs.victoriametrics.com/vmagent.html): introduce `remoteWrite.disableOnDiskQueue` flag which disables on-disk persistence and immediately returns an error to the push clients if in-memory queue is full. See [this feature request](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/2110).
