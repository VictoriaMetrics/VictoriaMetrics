...
* BUGFIX: `vmselect`: set proper timestamp for results returned for [instant queries](https://docs.victoriametrics.com/keyconcepts/#instant-query) that were optimized according to `--search.minWindowForInstantRollupOptimization`. Before, timestamps in results of such queries were shifted by `-search.cacheTimestampOffset`.
