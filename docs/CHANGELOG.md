...
* BUGFIX: [vmselect](https://docs.victoriametrics.com/Cluster-VictoriaMetrics.html): improve performance and memory usage during query processing on machines with big number of CPU cores by limiting the max number of parallel workers. The number of parallel workers for query processing can be controlled via `-search.maxWorkersPerQuery` command-line flag. See [this pull reuqest](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/5036) for details.
