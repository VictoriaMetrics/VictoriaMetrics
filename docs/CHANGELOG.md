...
* FEATURE: [vmagent]((https://docs.victoriametrics.com/vmagent.html)): add `-remoteWrite.removeDanglingQueues` command-line flag. It allows removing dangling queues from the disk when `vmagent` is restarted. This is useful when `-remoteWrite.url` is changed and the old queues are no longer needed. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/4014).
