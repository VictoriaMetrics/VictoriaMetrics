...
* SECURITY: add HSTS, CSP and X-Frame-Options headers to all http serving components with reasonable defaults. The values can be configured by users through new flags: `-http.header.hsts`, `-http.header.csp` and `-http.header.frameOptions`.
