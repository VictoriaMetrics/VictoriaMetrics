...
* BUGFIX: [vmselect](https://docs.victoriametrics.com/Cluster-VictoriaMetrics.html): keep metric name for time series stored in rollup cache on vmselect. Previously, incorrectly stored cache entries could cause a `duplicate series found` error during query processing. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/5332) for details.
