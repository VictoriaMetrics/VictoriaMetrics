...
* FEATURE: [vmalert](https://docs.victoriametrics.com/vmalert/): adds `datasource.idleConnTimeout` flag. Which must reduce probability of `broken pipe` and `connection reset by peer` errors. 
...
* FEATURE: [vmauth](https://docs.victoriametrics.com/vmauth/): adds `idleConnTimeout` flag. Which must reduce probability of `broken pipe` and `connection reset by peer` errors. 
