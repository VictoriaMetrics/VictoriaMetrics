...
* BUGFIX: [vmalert](https://docs.victoriametrics.com/vmalert.html): fix `endsAt` field in message that send to alertmanager, previously rule with small interval could never be triggered. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/5995) for details.
* BUGFIX: [vmalert](https://docs.victoriametrics.com/vmalert.html): fix wrong behavior of `-rule.resendDelay`, before it could prevent sending firing message when rule state is volatile. See [this pull request](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/6028) for details.
