...
* FEATURE: [vminsert](https://docs.victoriametrics.com/Cluster-VictoriaMetrics.html): sort labels in received data samples by default. This change updated default value of vminsert `-sortLabels` to `true`. This should make sharding of incoming time series across vmstorage shards consistent in VictoriaMetrics cluster. 
