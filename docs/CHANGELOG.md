...
* BUGFIX: [vmagent](https://docs.victoriametrics.com/vmagent.html): according to [the docs](https://docs.victoriametrics.com/vmagent.html#adding-labels-to-metrics) labels from `remoteWrite.label` are now added to sent metrics just before they are pushed to `remoteWrite.url` (after all relabelings, including stream aggregation relabelings). In addition, it allows to add labels for identifying vmagent instance for using streaming aggregation in vmagents cluster mode. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/4247) and [these docs](https://docs.victoriametrics.com/stream-aggregation.html#cluster-mode) for more details.
