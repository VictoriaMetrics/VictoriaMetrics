...
* BUGFIX: [vmalert](https://docs.victoriametrics.com/vmalert.html): reset alert's `ActiveAt` and `Start` when it becomes firing from previous `Stabilizing`[maintained by `keep_firing_for`]. See [this pull request](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/5258) for details.
