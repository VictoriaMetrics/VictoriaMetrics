...
* FEATURE: [vmctl](https://docs.victoriametrics.com/vmctl.html): add `vm-native-step-interval` command line flag for `vm-native` mode. Allows to split import process into smaller batches to improve
  process observability and add ability to restore process in case of failure. See [feature request](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/2733).
