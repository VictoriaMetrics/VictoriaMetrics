...
* FEATURE: [vmalert](https://docs.victoriametrics.com/#vmalert): add `type` query parameter for `/vmalert/groups`, `/rules`, `/vmalert/api/v1/rules` and `/api/v1/rules` that filter the rules by `record`, for record-rules or `alert` for alerts. See [the pull request](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/5749).
