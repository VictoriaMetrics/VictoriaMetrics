...
* FEATIRE: [vmgateway](https://docs.victoriametrics.com/vmgateway.html): define HTTP header name to look for JWT authorization token. It helps in such a case when user can't pass a jwt token at the same time because it should be passed in the same http header. See [feature request](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/3054).
