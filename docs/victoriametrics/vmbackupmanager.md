...
1. Ensure there is an init container with `vmbackupmanager restore` in `vmstorage` or `vmsingle` pod.
   ...
   See the operator `VMStorage` [schema](https://docs.victoriametrics.com/operator/api.html#vmstorage) and `VMSingle` [spec](https://docs.victoriametrics.com/operator/api.html#vmsinglespec).
...
1. Create a new cluster with access to *source* cluster `vmbackupmanager` storage and same number of storage nodes.
   Add the following section in order to enable restore on start. For additional information, see [`VMStorage` operator schema](https://docs.victoriametrics.com/operator/api.html#vmstorage):
