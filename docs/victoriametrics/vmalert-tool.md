...
The test harness seeds the test clock at `2000-01-01T00:00:00Z` (Unix timestamp `946684800`).
This means that `time()` returns `946684800` at `eval_time: 0`. When writing unit tests
with `input_series` values that represent absolute timestamps (e.g. metrics named
`*_timestamp_seconds`, `*_last_run`, etc.), add `946684800` to the seed values to match
the new test start time.
