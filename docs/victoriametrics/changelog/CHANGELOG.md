...
* FEATURE: [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/): add `__meta_hetzner_hcloud_location` and `__meta_hetzner_hcloud_location_network_zone` labels for `hcloud` role in `hetzner_sd_configs`, sourced from the new top-level `location` field in the Hetzner Cloud API. The existing `__meta_hetzner_hcloud_datacenter_location` and `__meta_hetzner_hcloud_datacenter_location_network_zone` labels are deprecated but still emitted for backward compatibility, now sourced from the same new field. **Note:** Hetzner will remove the `datacenter` field from the Hetzner Cloud API after 2026-07-01 (see [Hetzner changelog](https://docs.hetzner.cloud/changelog#2025-12-16-phasing-out-datacenters)), which means `__meta_hetzner_datacenter` will stop being populated for the `hcloud` role after that date. See [#10909](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10909).
* FEATURE: [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/): add `__meta_hetzner_robot_datacenter` label for `robot` role in `hetzner_sd_configs`. The existing `__meta_hetzner_datacenter` label is deprecated for this role but still emitted for backward compatibility. See [#10909](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10909).
