...
* FEATURE: all VictoriaMetrics components: add `-http.maxConns` command-line flag for limiting the number of established connections accepted at the corresponding `-httpListenAddr`. Connections exceeding the limit are closed immediately and counted via the new `vm_tcplistener_conns_dropped_total` metric. This helps protect against connection-exhaustion DoS and excessive CPU usage from misbehaving clients, for example during TLS handshakes at `vmagent`. See [#9776](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/9776).
