...
## tip
* FEATURE: [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/): introduce a new flag `-remoteWrite.enableRerouting` to explicitly control the rerouting behavior when there's any remote storage system can not keep up ingestion rate. Previously, it will be calculated internally base on the value of `-remoteWrite.disableOnDiskQueue` See [#10507](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10507). Thanks to @evkuzin for the contribution.
