...
* BUGFIX: `vmselect` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): cancel outstanding `vmstorage` search RPCs once their results are no longer needed when `-search.skipSlowReplicas` is enabled. This prevents connection spikes to slow or overloaded `vmstorage` nodes. See [#11034](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11034).
