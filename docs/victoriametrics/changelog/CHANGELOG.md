...
* BUGFIX: [vmsingle](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/) and `vmselect` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): fix instant rollup result cache keying for requests with `extra_label` by using request-stable cache filters. This prevents cache entry collisions across different filter contexts that could lead to pathological query latency. See [#10700](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/10700).
