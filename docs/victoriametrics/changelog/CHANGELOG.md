...
* FEATURE: [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/): add `vmauth_user_request_body_buffered_bytes` and `vmauth_unauthorized_user_request_body_buffered_bytes` metrics for tracking per-user request body memory buffered in vmauth. This allows identifying users with the highest memory usage via `topk(10, vmauth_user_request_body_buffered_bytes)`. See [#10683](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10683).
