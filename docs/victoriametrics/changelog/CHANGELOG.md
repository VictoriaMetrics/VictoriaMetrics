...
* BUGFIX: [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/): align request body buffering flags - `maxRequestBodySizeToRetry` and `requestBufferSize` to the same `16KB` value. And decouple buffering and request retry logic. See [#10675](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10675)
