...
* FEATURE: all VictoriaMetrics components: publish [CycloneDX](https://cyclonedx.org/) SBOMs (Software Bill of Materials) for container images. SBOMs are generated using [syft](https://github.com/anchore/syft) and attached to Docker images via [oras](https://oras.land/) for both Alpine and scratch variants across `docker.io` and `quay.io` registries. SBOM failures are non-blocking to avoid stopping releases. See [SECURITY.md](https://docs.victoriametrics.com/victoriametrics/security/) and [#10474](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/10474).
