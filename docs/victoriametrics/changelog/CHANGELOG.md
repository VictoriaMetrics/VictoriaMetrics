...
* BUGFIX: [vmsingle](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/) and `vmselect` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): accept `start` and `end` query args written in scientific notation with sub-second precision such as `1.784144612388E9`, for compatibility with Prometheus. Previously such values were rejected with an `HTTP 422` error, while the equivalent `1784144612.388` was accepted. See [#11268](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11268).
