...
* BUGFIX: [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/): keep the letter case of header names configured via `headers` and `response_headers` instead of canonicalizing them, so backends that require exact casing receive the configured names. See [#11222](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11222).
