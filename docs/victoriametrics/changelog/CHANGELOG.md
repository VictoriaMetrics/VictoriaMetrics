...
* FEATURE: [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/): add `deny_paths` option to `url_map` entries in auth config. Requests matching the routing conditions of a `url_map` entry and at least one of its `deny_paths` entries are rejected with `403 Forbidden` HTTP status code instead of being proxied to the backend. See [#11452](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11452).
