...
* FEATURE: [vmrestore](https://docs.victoriametrics.com/victoriametrics/vmrestore/): add `-restoreSince` and `-restorePartitions` command-line flags for partial restore from backup. `-restoreSince` restores only the partitions whose end date, derived from their `YYYY_MM` name, is newer than the given duration (e.g. `-restoreSince=5d` restores partitions covering the last 5 days); the whole partition is restored, even though it may also contain older data. `-restorePartitions` accepts a comma-separated list of `YYYY_MM` partition names to restore. Both flags help avoid over-provisioning disk space when only recent data is needed. Non-partition files (metadata, etc.) are always restored regardless of these flags. See [#10916](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10916).
* FEATURE: [vmrestore](https://docs.victoriametrics.com/victoriametrics/vmrestore/): add `-restoreSince` and `-restorePartitions` command-line flags for partial restore from backup. `-restoreSince` skips partitions older than the given duration (e.g. `-restoreSince=5d` restores only the last 5 days). `-restorePartitions` accepts a comma-separated list of `YYYY_MM` partition names to restore. Both flags help avoid over-provisioning disk space when only recent data is needed. Non-partition files (metadata, etc.) are always restored regardless of these flags. See [#10916](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10916).
