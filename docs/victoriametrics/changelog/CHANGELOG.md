...
* FEATURE: [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/): add support for configuring `Accept-Encoding` header dynamically via `accept_encoding` directive in [scrape_configs](https://docs.victoriametrics.com/victoriametrics/sd_configs/#scrape_configs). This allows targets to be scraped using modern compression algorithms like `zstd`, `gzip` or `identity` (uncompressed). See [#11227](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11227).
