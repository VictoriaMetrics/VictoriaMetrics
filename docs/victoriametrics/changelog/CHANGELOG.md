...
* FEATURE: [vmsingle](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/), [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/) and `vminsert` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): reduce peak memory usage when ingesting New Relic infrastructure events by passing converted rows to the insertion callback in approximately 4 MiB batches. The `vm_rows_per_insert{type="newrelic"}` and `vmagent_rows_per_insert{type="newrelic"}` histograms now record rows per batch instead of rows per request. If a later batch fails, earlier batches may have already been inserted. See [#11544](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11544).
