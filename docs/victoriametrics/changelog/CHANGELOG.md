...
* FEATURE: [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/): add `-remoteWrite.mdx.enable` command-line flag, which instructs `vmagent` to forward only metrics from VictoriaMetrics instances to the corresponding `-remoteWrite.url`. An instance is considered a VictoriaMetrics instance if it has previously emitted the `vm_app_version` metric. The TTL for tracked instances can be configured via `-remoteWrite.mdx.instanceTTL`. See [docs](https://docs.victoriametrics.com/victoriametrics/vmagent/#filtering-metrics-by-victoriametrics-instance) for details. Also see issue [#10600](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10600).
