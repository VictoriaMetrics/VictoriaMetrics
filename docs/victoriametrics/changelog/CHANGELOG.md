...
* BUGFIX: [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/) and [vmsingle](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/): stop scraping targets that were discovered by a service discovery config which has been removed from a scrape job on config reload. Previously, when the last `http_sd_configs` entry (or the last entry of any other `*_sd_configs` type except `file_sd_configs`, `static_configs` and `kubernetes_sd_configs`) was removed from a job, for example when switching the job from `http_sd_configs` to `file_sd_configs`, the targets from the previous discovery round were kept as if discovery had temporarily failed, and they continued to be scraped until the process was restarted. The regression was introduced in [v1.130.0](https://docs.victoriametrics.com/victoriametrics/changelog/changelog_2025/#v11300) by the fix for [#9949](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/9949).
