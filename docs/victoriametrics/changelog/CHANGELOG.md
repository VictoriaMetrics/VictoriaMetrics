...
* BUGFIX: [vmalert-tool](https://docs.victoriametrics.com/victoriametrics/vmalert-tool/): fix broken error formatting when `-external.url` is invalid, so the real parse error is shown instead of `%!w(...)`.
