...
* FEATURE: [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/): add `deny_paths` option to `url_map`, for rejecting specific paths matched by `src_paths` (or other `src_*` options) without listing every allowed path. A request matching both `src_paths` and `deny_paths` gets rejected with `403 Forbidden`. See [#11452](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11452).
