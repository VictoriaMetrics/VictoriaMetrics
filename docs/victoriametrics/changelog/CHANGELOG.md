...
* BUGFIX: [vmsingle](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/) and `vmselect` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): stop returning the `duplicate output timeseries` error for queries such as [label_replace](https://docs.victoriametrics.com/victoriametrics/metricsql/#label_replace) when the resulting series have identical labels but no samples at the same timestamp. Such series are now merged into a single series, like Prometheus does. The error is still returned when two series hold a sample at the same timestamp. See [#5138](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/5138).
