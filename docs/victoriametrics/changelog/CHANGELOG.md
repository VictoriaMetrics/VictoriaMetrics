...
* FEATURE: `vmselect` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): add `-clusterSelectNode` command-line flag for HTTP-based [multi-level cluster setup](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/#multi-level-cluster-setup). This replaces the legacy TCP-based `-clusternativeListenAddr` approach with standard HTTP API communication between upper-level and lower-level `vmselect` nodes, including correct `isPartial` propagation. See [#10905](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10905).
