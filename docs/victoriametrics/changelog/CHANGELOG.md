...
* BUGFIX: `vmselect` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): apply the `le` filter of [/api/v1/status/metric_names_stats](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/#prometheus-querying-api-usage) globally after merging the records from all the `vmstorage` nodes. Previously the `le` filter was applied on each `vmstorage` node independently, so a metric with a non-zero requests count on a subset of nodes could be reported with `queryRequestsCount: 0` and treated as never queried. See [#11473](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11473).
