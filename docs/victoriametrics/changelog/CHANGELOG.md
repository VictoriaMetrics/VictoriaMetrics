...
* BUGFIX: [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/) and [vmsingle](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/): cap `scrape_timeout` by the effective per-target `scrape_interval` after the `__scrape_interval__` / `__scrape_timeout__` labels are resolved. Previously the timeout was capped by the job-level or global interval at config load time, so a job-level `scrape_timeout` was silently lowered even when targets provided a longer interval via service discovery (e.g. `http_sd_configs`), and a label-provided `__scrape_timeout__` bigger than the interval was accepted as is. Now the timeout is validated against the final interval for every target, and a warning is logged when it gets capped. Targets providing a non-positive `__scrape_interval__` or `__scrape_timeout__` are rejected. See [#11597](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11597).
...
* BUGFIX: [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/): now [MDX](https://docs.victoriametrics.com/victoriametrics/vmagent/#monitoring-data-exchange) discards [cardinality_estimate](https://docs.victoriametrics.com/victoriametrics/vmestimator/#cardinality-metrics) and `cardinality_churn_ratio` metrics exposed by [vmestimator](https://github.com/VictoriaMetrics/vmestimator) service. [#11501](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11501).
* BUGFIX: [vmsingle](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/), `vmselect` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): prevent the process from crashing when [limit_offset](https://docs.victoriametrics.com/victoriametrics/metricsql/#limit_offset) is called with a negative `limit` or `offset` argument. See [#11561](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/11561). Thanks to @theoneLee for contribution. 
...
## [v1.148.4](https://github.com/VictoriaMetrics/VictoriaMetrics/releases/tag/v1.148.4)
