...
* FEATURE: all VictoriaMetrics components: expose `process_cpu_cores_host`, `process_cpu_cores_cgroup_quota`, `process_memory_host_bytes` and `process_memory_cgroup_bytes` metrics. Cgroup metrics are `-1` if no cgroup limit is set. See [#11460](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11460).
