...
**Update Note 1:** `vmselect` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): joins multiple `vm_account_id` and `vm_project_id` filters into a single filter at `extra_label` and `extra_filters[]` args for for [multitenant](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/#multitenancy) requests.
...
* BUGFIX: `vmselect` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): fix incorrect tenant selection logic for [multitenant](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/#multitenancy) API requests when both `extra_filters[]` and `extra_label` filters are specified. Previously, these filters were combined using `OR` logic instead of `AND`, which could allow authenticated users to query data outside their authorized tenant scope. Now `extra_label=vm_account_id=5` combined with `extra_filters[]={vm_account_id="10"}` correctly returns an empty response.
