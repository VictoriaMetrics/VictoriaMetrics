...
* FEATURE: [OpenTelemetry data ingestion](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/#sending-data-via-opentelemetry): reduce CPU usage when parsing metrics received via OpenTelemetry protocol.
...
* FEATURE: [vmalert](https://docs.victoriametrics.com/victoriametrics/vmalert/): add `search` query argument for search and `group_limit`, `group_next_token`, `group_before_token` query arguments for pagination support at `/api/v1/rules` API. See [#9580](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/9580).
