...
* FEATURE: [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/): support configuring request headers from JWT `vm_access` claim placeholders such as `{{.MetricsAccountID}}` and `{{.MetricsProjectID}}`. This simplifies JWT-based multitenant setups that use header-based tenant identification.
