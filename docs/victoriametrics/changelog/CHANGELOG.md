...
* FEATURE: [vmalert-tool](https://docs.victoriametrics.com/victoriametrics/vmalert-tool/): add the `--startTime` command-line flag for overriding the time `input_series` are seeded at and the first rule evaluation happens at. It defaults to `2000-01-01T00:00:00Z` and allows migrating test files which carry absolute timestamps compared against `time()` on your own schedule. See [#11437](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11437).
