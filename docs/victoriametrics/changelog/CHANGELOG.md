...
* FEATURE: [Stream aggregation](https://docs.victoriametrics.com/victoriametrics/stream-aggregation/): add `vm_streamaggr_output_state_size_bytes` and `vm_streamaggr_output_state_items_count` metrics for tracking the approximate memory usage and the number of tracked series per configured output (e.g. `total`, `rate_sum`, `histogram_bucket`). This helps identifying which output contributes the most to the memory usage of streaming aggregation. See [#9623](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/9623).
