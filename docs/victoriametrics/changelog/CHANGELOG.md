...
* FEATURE: [vmsingle](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/) and `vmstorage` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-victoriametrics/): hint the OS with `fadvise(FADV_RANDOM)` that data files are read at random offsets during queries. This disables OS readahead for reads performed through file descriptors and reduces disk read amplification on hosts with high `read_ahead_kb` settings. The hint can be disabled with the `-fs.disableFadviseRandomRead` command-line flag. See [#11461](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11461).
