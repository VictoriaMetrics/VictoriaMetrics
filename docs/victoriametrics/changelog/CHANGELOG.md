...
* BUGFIX: all VictoriaMetrics components: avoid logging PROXY protocol read errors when a connection times out before sending any data, such as AWS Network Load Balancer TLS listener connectivity tests. Timeouts after receiving part of the PROXY protocol message are still logged. See [#11600](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11600).
