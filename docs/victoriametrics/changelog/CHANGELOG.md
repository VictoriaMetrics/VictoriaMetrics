...
* BUGFIX: [MetricsQL](https://docs.victoriametrics.com/victoriametrics/metricsql/): `histogram_quantile` and `histogram_share` now return an error for histograms with overlapping buckets instead of returning a silently miscalculated result. Overlapping buckets occur when histograms with different bucket boundaries are aggregated together. See [#4037](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/4037) and [#10506](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/10506).
