...
* BUGFIX: all VictoriaMetrics components: create the Unix domain socket for `-httpListenAddr=unix:/path/to/socket` with `0660` permissions [as documented](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/#security). Previously it was created with `0600` permissions, so processes running under other users, such as a reverse proxy, couldn't connect to it even if they belong to the group of the socket file. See [#11615](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11615).
