...
* BUGFIX: [vmalert](https://docs.victoriametrics.com/victoriametrics/vmalert/): retry applying the [notifier configuration file](https://docs.victoriametrics.com/victoriametrics/vmalert/#notifier-configuration-file) on the next reload if the previous reload failed to start notifiers, for example due to a temporary service discovery error. Previously, vmalert treated the failed config as already applied, so some notifiers could stay missing until the file changed or vmalert restarted. See [#11645](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/11645).
