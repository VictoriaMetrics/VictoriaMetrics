...
* BUGFIX: [MetricsQL](https://docs.victoriametrics.com/victoriametrics/metricsql/): return an error instead of crashing the process when `limit_offset` is called with a negative `limit` or `offset` argument. Previously, a negative value reached the slice expression and panicked with `runtime error: slice bounds out of range`.
