...
... without listing every allowed path. A request matching both `src_paths` and `deny_paths` gets rejected with `403 Forbidden` (or `401 Unauthorized` for anonymous requests, to avoid confirming that the path is denied). See [#11452](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11452).
