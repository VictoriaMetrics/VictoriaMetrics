...
* BUGFIX: all VictoriaMetrics components: cancel in-flight HTTP requests once `-http.maxGracefulShutdownDuration` elapses during graceful shutdown, giving them up to 2 extra seconds to drain instead of waiting and then exiting via `logger.Fatalf` -> `os.Exit` on timeout. This prevents skipping the storage flush and losing in-memory data when long-lived requests are in flight (such as VictoriaLogs live tailing). See [#1502](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1502).
