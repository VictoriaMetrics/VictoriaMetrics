...
* FEATURE: [metrics metadata](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/#metrics-metadata): save the metadata cache on graceful shutdown and restore it on startup, preserving the one-hour expiration since the last ingestion. See [#11609](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11609).
