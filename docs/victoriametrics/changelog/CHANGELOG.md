...
* BUGFIX: [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/): fix the bug that could cause time series array from being not returned to the object pool and resued after processing remote write data, the bug could increase CPU and memory usage if vmagent is configured with per-URL relabeling via `-remoteWrite.urlRelabelConfig` or per-URL stream aggregation via `-remoteWrite.streamAggr.config`. See [#11250](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11250).
