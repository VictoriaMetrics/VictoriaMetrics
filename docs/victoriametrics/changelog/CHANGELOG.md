...
* FEATURE: [vmalert](https://docs.victoriametrics.com/victoriametrics/vmalert/): start groups with `group.interval > 1m` faster. Before, vmalert was [delaying group start](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/724) for random duration up to `group.interval`. This could significantly delay group's evaluation if their interval were set to big values. With this change, vmalert will start all the groups on `1m` interval or faster.
