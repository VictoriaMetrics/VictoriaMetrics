...
* BUGFIX: [MetricsQL](https://docs.victoriametrics.com/victoriametrics/metricsql/): `histogram_quantile` and `histogram_share` now return `NaN` for histograms with overlapping buckets instead of returning a silently miscalculated result. Overlapping buckets occur when histograms with different bucket boundaries are aggregated together. See [#4037](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/4037).
