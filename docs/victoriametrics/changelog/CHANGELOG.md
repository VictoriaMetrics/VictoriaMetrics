...
## tip
* FEATURE: show how the default value is calculated for command-line flags which derive it from the number of available CPU cores. `-help` output and the flag docs generated from it now print `(default 2 * availableCPUs)` for `-maxConcurrentInserts` instead of a number which only holds for the machine that generated the docs. The same applies to `-search.maxConcurrentRequests`, `-fs.maxConcurrency` and `-remoteWrite.concurrency` in [vmalert](https://docs.victoriametrics.com/victoriametrics/vmalert/). See [#9680](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/9680).
