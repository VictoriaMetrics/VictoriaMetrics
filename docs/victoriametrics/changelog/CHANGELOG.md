...
* FEATURE: [vmalert-tool](https://docs.victoriametrics.com/victoriametrics/vmalert-tool/): add the `--startTime` command-line flag for overriding the time `input_series` are seeded at and the first rule evaluation happens at. It defaults to `2000-01-01T00:00:00Z` and allows migrating test files which carry absolute timestamps compared against `time()` on your own schedule. Start times outside the range the underlying storage accepts samples at are rejected with an explanatory error instead of running a test which silently sees no input at all. See [#11437](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11437).
* FEATURE: [vmalert-tool](https://docs.victoriametrics.com/victoriametrics/vmalert-tool/): support the `start_timestamp` option in `<test_group>`, the same way [promtool](https://prometheus.io/docs/prometheus/latest/configuration/unit_testing_rules/#test_group) does. It accepts either a Unix timestamp in seconds such as `1609459200` or an RFC3339 timestamp such as `"2021-01-01T00:00:00Z"`, and takes precedence over the `--startTime` command-line flag, so a single group can pin the timestamps its `input_series` were written against while the flag moves the rest of the suite. See [#11437](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11437).
...
* BUGFIX: [vmalert-tool](https://docs.victoriametrics.com/victoriametrics/vmalert-tool/): document the unit test start time in [vmalert-tool docs](https://docs.victoriametrics.com/victoriametrics/vmalert-tool/#test-start-time) and refresh the [debug mode](https://docs.victoriametrics.com/victoriametrics/vmalert-tool/#debug-mode) sample output, which still showed the `1970-01-01` timestamps used before [v1.148.0](https://docs.victoriametrics.com/victoriametrics/changelog/#v11480). See [#11437](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/11437).
...
**Update Note 1:** [vmalert-tool](https://docs.victoriametrics.com/victoriametrics/vmalert-tool/): unit tests are now seeded starting at `2000-01-01T00:00:00Z` instead of `1970-01-01T00:00:00Z`, because the first day of the Unix epoch is reserved for the global index search. See [#11219](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/11219). This is a breaking change for test files whose `input_series` hold absolute Unix timestamps compared against `time()`, since `eval_time: 0` no longer coincides with timestamp `0`. To migrate such a file, add `946684800` to every absolute seed value while keeping its increment and repeat count - `0x121` becomes `946684800x121`, and `0+60x60` becomes `946684800+60x60`. Values which are counters, gauges or booleans need no change. `eval_time` also has to be recomputed for rules which use `day_of_week()`, `hour()` or `minute()`, because `2000-01-01` is a Saturday while `1970-01-01` was a Thursday. Starting from [tip](https://docs.victoriametrics.com/victoriametrics/changelog/#tip) the start time can be pinned via the `--startTime` command-line flag while such files are being migrated.
...
**Update Note 1:** [vmalert-tool](https://docs.victoriametrics.com/victoriametrics/vmalert-tool/): unit tests are now seeded starting at `2000-01-01T00:00:00Z` instead of `1970-01-01T00:00:00Z`. See [#11219](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/11219). This is a breaking change for test files whose `input_series` hold absolute Unix timestamps compared against `time()`. See the migration steps in [Update Note 1 for v1.148.0](https://docs.victoriametrics.com/victoriametrics/changelog/#v11480).
