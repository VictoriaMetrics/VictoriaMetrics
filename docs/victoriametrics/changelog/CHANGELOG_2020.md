...
* FEATURE: add `/tags/delSeries` handler from Graphite Tags API. See <https://docs.victoriametrics.com/victoriametrics/single-node-version/#graphite-tags-api-usage>
...
* FEATURE: added `/internal/force_flush` http handler for flushing recently ingested data from in-memory buffers to persistent storage.
  See [troubleshooting docs](https://docs.victoriametrics.com/victoriametrics/single-node-version/#troubleshooting) for more details.
* FEATURE: added [Graphite Tags API](https://graphite.readthedocs.io/en/stable/tags.html) support.
  See [these docs](https://docs.victoriametrics.com/victoriametrics/single-node-version/#graphite-tags-api-usage) for details.
...
* FEATURE: add ability to export data in CSV format. See [these docs](https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-export-csv-data) for details.
...
* FEATURE: add `vm_rows_added_to_storage_total` metric, which shows the total number of rows added to storage since app start.
  ...
  due to [relabeling](https://docs.victoriametrics.com/victoriametrics/single-node-version/#relabeling). The `sum(rate(vm_rows_added_to_storage_total))` can be bigger
  than `sum(rate(vm_rows_inserted_total))` if [replication](https://docs.victoriametrics.com/victoriametrics/cluster-version/#replication-and-data-safety) is enabled.
...
* FEATURE: use all the available CPU cores when accepting data via a single TCP connection
  for [all the supported protocols](https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-import-time-series-data).
...
* FEATURE: add `reduce_mem_usage` query option to `/api/v1/export` in order to reduce memory usage during data export / import.
  See [these docs](https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-export-data-in-json-line-format) for details.
...
* BUGFIX: properly handle case when [/metrics/find](https://docs.victoriametrics.com/victoriametrics/single-node-version/#graphite-metrics-api-usage) finds both a leaf and a node for the given `query=prefix.*`.
