...
where `timestamp` equals the `time` query arg, while the `value` contains `query` result at the requested `time`.
...
It can be overridden on a per-query basis via `latency_offset` query arg.
