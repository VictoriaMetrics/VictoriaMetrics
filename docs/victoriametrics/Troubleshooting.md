...
1. Check the version of VictoriaMetrics component, which needs to be troubleshot and compare
   ...
   - [How to upgrade single-node VictoriaMetrics](https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-upgrade-victoriametrics)
   - [How to upgrade VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-version/#updating--reconfiguring-cluster-nodes)
...
1. If the simplest query continues returning unexpected / unreliable results, then try verifying correctness
   of raw unprocessed samples for this query via [/api/v1/export](https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-export-data-in-json-line-format)
   ...
   for details. The raw samples can be also viewed in [vmui](https://docs.victoriametrics.com/victoriametrics/single-node-version/#vmui) in `Raw Query` tab and shared via `export` button.
...
1. Sometimes response caching may lead to unexpected results when samples with older timestamps
   are ingested into VictoriaMetrics (aka [backfilling](https://docs.victoriametrics.com/victoriametrics/single-node-version/#backfilling)).
...
1. If you use cluster version of VictoriaMetrics, then it may return partial responses by default
   when some of `vmstorage` nodes are temporarily unavailable - see [cluster availability docs](https://docs.victoriametrics.com/victoriametrics/cluster-version/#cluster-availability)
...
1. Try executing the query with `trace=1` query arg. This enables query tracing, which may contain
   useful information on why the query returns unexpected data. See [query tracing docs](https://docs.victoriametrics.com/victoriametrics/single-node-version/#query-tracing) for details.
...
1. If the steps above didn't help identifying the root cause of unexpected query results,
   ...
   Instead of sharing screenshots in the issue, consider sharing query and [trace](https://docs.victoriametrics.com/victoriametrics/single-node-version/#query-tracing) 
   results in [VMUI](https://docs.victoriametrics.com/victoriametrics/single-node-version/#vmui) by clicking on `Export query` button in top right corner of the graph area.
...
1. Memory shortage for the given amounts of [active time series](https://docs.victoriametrics.com/victoriametrics/faq/#what-is-an-active-time-series).
   ...
   The [official Grafana dashboards for VictoriaMetrics](https://docs.victoriametrics.com/victoriametrics/single-node-version/#monitoring)
   ...
   - To reduce the number of active time series. The [official Grafana dashboards for VictoriaMetrics](https://docs.victoriametrics.com/victoriametrics/single-node-version/#monitoring)
     contain a graph showing the number of active time series. Recent versions of VictoriaMetrics
     provide [cardinality explorer](https://docs.victoriametrics.com/victoriametrics/single-node-version/#cardinality-explorer),
...
1. [High churn rate](https://docs.victoriametrics.com/victoriametrics/faq/#what-is-high-churn-rate),
   ...
   The [cardinality explorer](https://docs.victoriametrics.com/victoriametrics/single-node-version/#cardinality-explorer) can help identifying
   such labels.

1. Resource shortage. The [official Grafana dashboards for VictoriaMetrics](https://docs.victoriametrics.com/victoriametrics/single-node-version/#monitoring)
   ...
   - If free disk space is lower than 20%, then VictoriaMetrics is unable to perform optimal
     ...
     which, in turn, slows down both data ingestion and querying. See [these docs](https://docs.victoriametrics.com/victoriametrics/single-node-version/#storage) for details.
...
1. If you run cluster version of VictoriaMetrics, then make sure `vminsert` and `vmstorage` components
   ...
   The [official Grafana dashboard for cluster version of VictoriaMetrics](https://docs.victoriametrics.com/victoriametrics/cluster-version/#monitoring)
...
1. Noisy neighbor. Make sure VictoriaMetrics components run in an environments without other resource-hungry apps.
   ...
   Issues like this are very hard to catch via [official Grafana dashboard for cluster version of VictoriaMetrics](https://docs.victoriametrics.com/victoriametrics/cluster-version/#monitoring)
   and proper diagnosis would require checking resource usage on the instances where VictoriaMetrics runs.

1. If you see `TooHighSlowInsertsRate` [alert](https://docs.victoriametrics.com/victoriametrics/single-node-version/#monitoring) when single-node VictoriaMetrics or `vmstorage` has enough
...
VictoriaMetrics provides [`top queries` page at VMUI](https://docs.victoriametrics.com/victoriametrics/single-node-version/#top-queries), which shows
...
- Adding more CPU and memory to VictoriaMetrics, so it may perform the slow query faster.
  ...
  If the line on `concurrent select` graph form the [official Grafana dashboard for VictoriaMetrics](https://docs.victoriametrics.com/victoriametrics/cluster-version/#monitoring)
...
- Rewriting slow queries, so they become faster. Unfortunately it is hard determining
  ...
  VictoriaMetrics provides [query tracing](https://docs.victoriametrics.com/victoriametrics/single-node-version/#query-tracing) feature,
...
1. Improper command-line flag values. Inspect command-line flags passed to VictoriaMetrics components.
   ...
   [These docs](https://docs.victoriametrics.com/victoriametrics/single-node-version/#cache-tuning) refer command-line flags, which aren't
...
1. Unexpected heavy queries. The query is considered as heavy if it needs to select and process millions of unique time series.
   ...
   VictoriaMetrics provides [various settings](https://docs.victoriametrics.com/victoriametrics/single-node-version/#resource-usage-limits), 
   ...
   VictoriaMetrics also provides [query tracer](https://docs.victoriametrics.com/victoriametrics/single-node-version/#query-tracing) 
...
1. Lack of free memory for processing workload spikes. If VictoriaMetrics components use almost all the available memory
   ...
   See [capacity planning for single-node VictoriaMetrics](https://docs.victoriametrics.com/victoriametrics/single-node-version/#capacity-planning)
   and [capacity planning for cluster version of VictoriaMetrics](https://docs.victoriametrics.com/victoriametrics/cluster-version/#capacity-planning).
...
- Workload spikes. For example, if the number of active time series increases by 2x while
  ...
  See [these docs](https://docs.victoriametrics.com/victoriametrics/cluster-version/#resource-usage-limits) for details.
...
- Various maintenance tasks such as rolling upgrades or rolling restarts during configuration changes.
  ...
  see [replication and data safety docs](https://docs.victoriametrics.com/victoriametrics/cluster-version/#replication-and-data-safety)
  for details.

- Time series sharding. Received time series [are consistently sharded](https://docs.victoriametrics.com/victoriametrics/cluster-version/#architecture-overview)
...
See [capacity planning docs](https://docs.victoriametrics.com/victoriametrics/cluster-version/#capacity-planning)
and [cluster resizing and scalability docs](https://docs.victoriametrics.com/victoriametrics/cluster-version/#cluster-resizing-and-scalability)
for details.
...
at [VictoriaMetrics cluster](https://docs.victoriametrics.com/victoriametrics/cluster-version/), then please check the following:

- Make sure that there are no old snapsots, since they can occupy disk space. See [how to work with snapshots](https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-work-with-snapshots)
  and [snapshot troubleshooting](https://docs.victoriametrics.com/victoriametrics/single-node-version/#snapshot-troubleshooting).
...
- Under normal conditions the size of `<-storageDataPath>/indexdb` folder must be smaller than the size of `<-storageDataPath>/data` folder, where `-storageDataPath`
  ...
  The solution is to identify and fix the source of high churn rate with [cardinality explorer](https://docs.victoriametrics.com/victoriametrics/single-node-version/#cardinality-explorer).


## Monitoring

Having proper [monitoring](https://docs.victoriametrics.com/victoriametrics/single-node-version/#monitoring)
