...
- Network instability between cluster components (`vminsert`, `vmselect`, `vmstorage`) may lead to increased error rates, timeouts, or degraded performance. 
  ...
  Consider increasing `-vmstorageDialTimeout` and `-rpc.handshakeTimeout` to mitigate the effects of CPU spikes.
  ...
  If the network cannot be improved, increasing timeouts such as `-vmstorageDialTimeout`, `-rpc.handshakeTimeout`, or `-search.maxQueueDuration` may help, but should be done cautiously, as higher timeouts can impact cluster stability in other ways.
