...
`vmauth` can authorize access to backends based on the [JWT token](https://www.jwt.io/) provided in the `Authorization` request header. 
...
To start using OIDC realms and clients, a required realm must be set. Realm defines a set of keys, which are either automatically discovered (`issuer_url` is set and `skip_discovery: false`)
or specified manually (`public_key_files`, `public_keys` or `jwks_url` are set and `skip_discovery: true`) to validate the signature of the incoming JWT token against.
...
which `issuer_url` should match the JWT `iss` claim) of a user who is allowed to access the vmauth backend.
...
If there's a need to override default claims for client ID and issuer, it's possible to do so using `oidc.claims.client_id` and `oidc.claims.issuer` parameters:
...
If the client's realm is not present at `oidc.realms`, then it uses the default realm if it's defined at `oidc.default_realm`:
...
Both `oidc.realms` and `oidc.default_realm` allow disabling signature verification using `skip_discovery: true` and with `public_keys`, `public_key_files`, and `jwks_url` not set.
...
* `iss` - optional, OIDC issuer URL, which is used for matching incoming request against `oidc.realms[*].issuer_url`, `oidc.default_realm` is used if this claim is omitted. Claim name can be overridden using `oidc.claims.issuer` configuration property
* `exp` - required, expiration time in unix_timestamp. If the token expires, then `vmauth` rejects the request.
...
* `query_args` - optional, key-value pairs, where key is a query argument name and value is a slice of values for additional arguments that can be referenced in `url_prefix` and `default_url` query arg params.

### Claim values templating

VMAuth allows data retrieved from `query_args` and `tenant_id` to be used in templates for vmauth backend URL configurations via the expression `{{ variable }}`.
...
```
and given below the vmauth config
...
> only issuer URL part of `--auth.oidcDiscoveryEndpoints` value should be moved into the `issuer_url` realm parameter (without `/.well-known/openid-configuration` suffix).
...
Also, in the configuration above, ingestion and query URLs have additional `/insert` and `/select` path prefixes, respectively.
