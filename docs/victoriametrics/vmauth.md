...
Claim names must point to a **leaf value**.
...
neither takes priority. Resolve this by adding a shared discriminating claim
(e.g. `iss`) to both users, or by combining the conditions into a single user
with a `url_map`.
