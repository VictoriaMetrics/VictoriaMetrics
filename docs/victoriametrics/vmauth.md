...
Review `unauthorized_user` rules carefully.
Requests without credentials are matched against `unauthorized_user`, even if a similar route exists under authenticated `users`.
