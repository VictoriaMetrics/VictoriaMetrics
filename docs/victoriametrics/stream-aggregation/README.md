...
- It can filter out raw samples matched by aggregation rules, so raw data will never reach remote destination. See `--streamAggr.keepInput` and `-streamAggr.dropInput` in [aggregation config](https://docs.victoriametrics.com/victoriametrics/stream-aggregation/configuration);
...
disk IO and network bandwidth for processing at metrics storage side. 

For example, if `http_request_duration_seconds` histogram is generated by thousands of application instances, 
...
But it only shifts slowness from alerting rule to recording rule, since calculation still has to happen somewhere. 
...
This query is executed much faster than the original query, because it needs to scan lower number of time series.

> `avg_over_time(<aggregate:1m>[5m])` is similar to recording rules calculating rate over sliding window of `5m` with `1m` interval.
  If sliding window isn't important, then simply just omit `avg_over_time` aggregation in expression. 
...
[vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/) supports [de-duplication](https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/#deduplication) of samples
