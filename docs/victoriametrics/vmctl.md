...
COMMANDS:
   ...
   verify-block  Verifies correctness of data blocks exported via VictoriaMetrics Native format. See https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-export-data-in-native-format
```
...
The simplest way to migrate data between VM instances is [to copy data between instances](https://docs.victoriametrics.com/victoriametrics/single-node-version/#data-migration).

vmctl uses [native binary protocol](https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-export-data-in-native-format)
...
1. Migrating big volumes of data may result in reaching the safety limits on `src` side.
   ...
   [here](https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-export-data-in-native-format).
...
1. When migrating data from one VM cluster to another, consider using [cluster-to-cluster mode](#cluster-to-cluster-migration-mode).
   Or manually specify addresses according to [URL format](https://docs.victoriametrics.com/victoriametrics/cluster-version/#url-format):
   ```sh
...
1. Migrating data from VM cluster which had replication (`-replicationFactor` > 1) enabled won't produce the same amount
   ...
   See more about replication [here](https://docs.victoriametrics.com/victoriametrics/cluster-version/#replication-and-data-safety).
...
[native format](https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-export-data-in-native-format)
...
./vmctl vm-native -help
   ...
   --vm-native-filter-time-start value
     The time filter may contain different timestamp formats. See more details here https://docs.victoriametrics.com/victoriametrics/single-node-version/#timestamp-formats
   ...
   --vm-native-src-addr value
     ...
     Should be the same as --httpListenAddr value for single-node version or vmselect component. If exporting from cluster version see https://docs.victoriametrics.com/victoriametrics/cluster-version/#url-format
   ...
   --vm-native-dst-addr value
     ...
     Should be the same as --httpListenAddr value for single-node version or vminsert component. 
     If importing into cluster version see https://docs.victoriametrics.com/victoriametrics/cluster-version/#url-format
   ...
   --vm-native-disable-binary-protocol
     Whether to use https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-export-data-in-json-line-format instead of 
     https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-export-data-in-native-format API. 
