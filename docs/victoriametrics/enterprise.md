...
Enterprise binaries and packages have `enterprise` suffix in their names. For example, `victoria-metrics-linux-amd64-v1.129.0-enterprise.tar.gz`.
...
```sh
wget https://github.com/VictoriaMetrics/VictoriaMetrics/releases/download/v1.129.0/victoria-metrics-linux-amd64-v1.129.0-enterprise.tar.gz
tar -xzf victoria-metrics-linux-amd64-v1.129.0-enterprise.tar.gz
...
Enterprise docker images have `enterprise` suffix in their names. For example, `victoriametrics/victoria-metrics:v1.129.0-enterprise`.
...
```sh
docker run --name=victoria-metrics victoriametrics/victoria-metrics:v1.129.0-enterprise -license=BASE64_ENCODED_LICENSE_KEY
```

Alternatively, the license code can be stored in the file and then referred via `-licenseFile` command-line flag:

```sh
docker run --name=victoria-metrics -v /vm-license:/vm-license  victoriametrics/victoria-metrics:v1.129.0-enterprise -licenseFile=/path/to/vm-license
```
...
`victoria-metrics-linux-amd64-v1.129.0-enterprise.tar.gz`
...
`victoriametrics/victoria-metrics:v1.129.0-enterprise-fips` – uses the FIPS-compatible binary and based on `scratch` image.
