...
* If the command line flag `-zabbixconnector.addGroupsValue=<value>` is specified, the elements of the `$.groups` group array will be converted to the label name prefixed with `group_` with the value `<value>`.
