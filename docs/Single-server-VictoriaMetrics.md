...
* [How to set up scraping of Prometheus-compatible targets](./#how-to-scrape-prometheus-exporters-such-as-node-exporter)
* [How to ingest data to VictoriaMetrics](#how-to-import-time-series-data)
* [How to set up Prometheus to write data to VictoriaMetrics](./#prometheus-setup)
...
When querying the [backfilled data](./#backfilling)
...
VMUI allows sharing query and [trace](./#query-tracing) results by clicking on
...
in [query APIs](./#prometheus-querying-api-usage) and
in [export APIs](./#how-to-export-time-series).
...
- `-search.ignoreExtraFiltersAtLabelsAPI` enables ignoring of `match[]`, [`extra_filters[]` and `extra_label`](./#prometheus-querying-api-enhancements)
...
- The data outside the configured retention isn't deleted instantly - it is deleted eventually during [background merges](./#storage).
...
Downsampling is performed during [background merges](./#storage).
...
In case [deduplication](./#deduplication) is enabled, value of `-dedup.minScrapeInterval` command-line flag must also
...
applied eventually to historical data during  [background merges](./#storage).
...
This functionality may be used instead of [classic Prometheus-like metrics scraping](./#how-to-scrape-prometheus-exporters-such-as-node-exporter)
...
```
  ...
  -dedup.minScrapeInterval duration
     Leave only the last sample in every time series per each discrete interval equal to -dedup.minScrapeInterval > 0. See also -streamAggr.dedupInterval and ./#deduplication
  -deleteAuthKey value
