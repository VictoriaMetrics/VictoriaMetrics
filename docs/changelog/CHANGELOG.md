...
* BUGFIX: [stream aggregation](https://docs.victoriametrics.com/stream-aggregation/): support having multiple labels per `remoteWrite.url` using `-remoteWrite.streamAggr.dropInputLabels`. Before, labels were set to all the `remoteWrite.url`. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/6780) for the details.
