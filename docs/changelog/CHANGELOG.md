...
**Update note 3: [vmalert](https://docs.victoriametrics.com/vmalert/) disallows specifying `eval_offset` and `eval_delay` options in the same [group](https://docs.victoriametrics.com/vmalert/#groups). Since `eval_offset` ensures the group is evaluated at the exact offset in the range of [0...interval], while `eval_delay` cannot guarantee this behavior without further modifications to the evaluation time, potentially leading to more confusion.**
