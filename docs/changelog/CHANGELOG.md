...
* BUGFIX: `vmstorage` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/cluster-victoriametrics/): Fix for cleaning up storage from old data during final deduplication for the case when the cluster deduplication period has increased and affects more than one partition. Previously, old merged partitions were not removed during the final deduplication process because they were locked until the end of the process. This could lead to unexpectedly running out of space. See this [PR](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/6958) for details.
