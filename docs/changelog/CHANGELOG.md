...
* FEATURE: [vmsingle](https://docs.victoriametrics.com/single-server-victoriametrics/): add new flag `--storage.trackMetricNamesUsage` and new HTTP API - `/api/v1/status/metric_names_usage_stats`. It allows to track metric names usage for ingest and query requests. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/4458) for details.
