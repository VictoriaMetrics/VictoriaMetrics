...
* FEATURE: `vmstorage` in [VictoriaMetrics cluster](https://docs.victoriametrics.com/cluster-victoriametrics/): automatically calculate the `maxUniqueTimeseries` based on available memory and concurrent requests if it is not set. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/6930).
