...
func TestSingleMetricNamesStats(t *testing.T) {
	...
	metricNamesStatsCmpOpts := []cmp.Option{
		cmpopts.IgnoreFields(at.MetricNamesStatsResponse{}, "Status", "StatsCollectedSince", "TrackerMemoryMaxSizeBytes", "TrackerCurrentMemoryUsageBytes"),
		cmpopts.IgnoreFields(at.MetricNamesStatsRecord{}, "LastQueryRequestTimestamp"),
	}
	// verify ingest request correctly registered
	expected := apptest.MetricNamesStatsResponse{
		StatsCollectedRecordsTotal: 3,
	...
	got := sut.TSDBAPIV1MetricNamesStats(t, "", "", "", at.QueryOpts{})
	if diff := cmp.Diff(expected, got, metricNamesStatsCmpOpts...); diff != "" {
	...
	got = sut.TSDBAPIV1MetricNamesStats(t, "", "2", "", at.QueryOpts{})
	if diff := cmp.Diff(expected, got, metricNamesStatsCmpOpts...); diff != "" {
		t.Errorf("unexpected response (-want, +got):\n%s", diff)
	}

	// reset state and check empty request response
	sut.AdminResetMetricNamesStats(t, at.QueryOpts{})
	expected = apptest.MetricNamesStatsResponse{
		Records: []at.MetricNamesStatsRecord{},
	}
	got = sut.TSDBAPIV1MetricNamesStats(t, "", "", "", at.QueryOpts{})
...
func TestClusterMetricNamesStats(t *testing.T) {
	...
	if resp.StatsCollectedSince == 0 {
		t.Fatalf("unexpected zero stats collected since")
	}
	...
	metricNamesStatsCmpOpts := []cmp.Option{
		cmpopts.IgnoreFields(at.MetricNamesStatsResponse{}, "Status", "StatsCollectedRecordsTotal", "StatsCollectedSince", "TrackerMemoryMaxSizeBytes", "TrackerCurrentMemoryUsageBytes"),
		cmpopts.IgnoreFields(at.MetricNamesStatsRecord{}, "LastQueryRequestTimestamp"),
	}
	...
	for _, tenantID := range tenantIDs {
		...
		if diff := cmp.Diff(expected, gotStats, metricNamesStatsCmpOpts...); diff != "" {
			t.Errorf("unexpected response (-want, +got):\n%s", diff)
		}
	...
	if resp.StatsCollectedRecordsTotal != 0 {
		t.Fatalf("want 0 stats collected records, got: %d", resp.StatsCollectedRecordsTotal)
	}
