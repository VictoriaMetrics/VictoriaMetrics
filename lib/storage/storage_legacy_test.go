...
func TestLegacyStorageSnapshots_CreateListDelete(t *testing.T) {
	...
	mrs := testGenerateMetricRowsWithPrefixForTenantID(rng, accountID, projectID, numRows, "metric", tr)
...
func TestStorageConvertToLegacy(t *testing.T) {
	...
	mrs1 := testGenerateMetricRowsWithPrefixForTenantID(rng, accountID, projectID, numSeries, "generation1", tr1)
	mrs2 := testGenerateMetricRowsWithPrefixForTenantID(rng, accountID, projectID, numSeries, "generation2", tr2)
	mrs3 := testGenerateMetricRowsWithPrefixForTenantID(rng, accountID, projectID, numSeries, "generation3", tr3)
	mrs4 := testGenerateMetricRowsWithPrefixForTenantID(rng, accountID, projectID, numSeries, "generation4", tr4)
...
func mustConvertToLegacy(s *Storage, accountID, projectID uint32) *Storage {
...
func TestLegacyStorageGetTSDBStatus(t *testing.T) {
	defer testRemoveAll(t)
	const (
		accountID = 2
		projectID = 3
	...
	mrsPrev := testGenerateMetricRowsWithPrefixForTenantID(rng, accountID, projectID, numMetricsPrev, "legacy_prev", trPrev)
	...
	mrsCurr := testGenerateMetricRowsWithPrefixForTenantID(rng, accountID, projectID, numMetricsCurr, "legacy_curr", trCurr)
	...
	mrsPt := testGenerateMetricRowsWithPrefixForTenantID(rng, accountID, projectID, numMetricsPt, "pt", trPt)
	...
	assertTSDBStatus := func(date uint64, want *TSDBStatus) {
		...
		if got.TotalSeries != want.TotalSeries {
			t.Fatalf("unexpected TotalSeries: got %d, want %d", got.TotalSeries, want.TotalSeries)
		}
		if got.TotalLabelValuePairs != want.TotalLabelValuePairs {
			t.Fatalf("unexpected TotalLabelValuePairs: got %d, want %d", got.TotalLabelValuePairs, want.TotalLabelValuePairs)
		}
	}

	// check legacy prev indexDB date
	assertTSDBStatus(datePrev, &TSDBStatus{
		TotalSeries:          numMetricsPrev,
		TotalLabelValuePairs: numMetricsPrev * 3,
	})

	// check legacy curr indexDB date
	assertTSDBStatus(dateCurr, &TSDBStatus{
		TotalSeries:          numMetricsCurr,
		TotalLabelValuePairs: numMetricsCurr * 3,
	})

	// check partition indexDB date
	assertTSDBStatus(datePt, &TSDBStatus{
		TotalSeries:          numMetricsPt,
		TotalLabelValuePairs: numMetricsPt * 3,
	})
}
