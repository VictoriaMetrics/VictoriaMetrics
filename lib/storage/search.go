...
func (s *Search) NextMetricBlock() bool {
	...
	for s.ts.NextBlock() {
		...
		if tsid.MetricID != s.prevMetricID {
			...
			var ok bool
			s.MetricBlockRef.MetricName, ok = s.searchMetricName(s.MetricBlockRef.MetricName[:0], tsid.MetricID, tsid.AccountID, tsid.ProjectID)
			if !ok {
...
func (s *Search) searchMetricName(metricName []byte, metricID uint64, accountID, projectID uint32) ([]byte, bool) {
	mn := s.storage.getMetricNameFromCache(metricName, metricID)
	if len(mn) > len(metricName) {
		return mn, true
	}

	mn, found := s.idbCurr.searchMetricName(metricName, metricID, accountID, projectID, true)
	if found {
	...
	mn, found = s.idbPrev.searchMetricName(metricName, metricID, accountID, projectID, true)
	if found {
		s.storage.putMetricNameToCache(metricID, mn)
		return mn, true
	}
