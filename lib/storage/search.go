...
func (s *Search) searchMetricName(metricName []byte, metricID uint64, accountID, projectID uint32, bh *blockHeader) ([]byte, bool) {
	...
	if idbCurr := s.legacyIDBs.getIDBCurr(); idbCurr != nil {
		mn, found := idbCurr.searchMetricName(metricName, metricID, accountID, projectID, false)
		if found {
			return mn, true
		}
	}
	...
	if idbPrev := s.legacyIDBs.getIDBPrev(); idbPrev != nil {
		mn, found := idbPrev.searchMetricName(metricName, metricID, accountID, projectID, false)
		if found {
			return mn, true
		}
	}
