...
func TestSearchTenantsOnDate(t *testing.T) {
	...
	ptw := s.tb.MustGetPartition(baseTime.UnixMilli())
	defer s.tb.PutPartition(ptw)
	...
	is2 := db.getIndexSearch(0, 0, noDeadline)
	tenantsMap, err := is2.searchTenantsOnDate(date1)
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
	...
	tenantsMap2, err := is2.searchTenantsOnDate(date2)
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
	...
	// Test searching date `0`, should return all tenants
	tenantsMap3, err := is2.searchTenantsOnDate(0)
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
	expectedTenants3 := []string{"1:10", "1:11", "2:20", "2:21", "3:30"}
	if len(tenantsMap3) != len(expectedTenants3) {
		t.Fatalf("unexpected tenants count for date `0`; got %d; want %d", len(tenantsMap3), len(expectedTenants3))
	}
	for _, tenant := range expectedTenants3 {
		if _, ok := tenantsMap3[tenant]; !ok {
			t.Fatalf("expected tenant %s not found for date `0`", tenant)
		}
	}
	db.putIndexSearch(is2)
}
