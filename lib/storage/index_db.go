...
type indexDB struct {
	...
	tagFiltersToMetricIDsCache *tagFiltersCache
...
type IndexDBMetrics struct {
	...
	TagFiltersToMetricIDsCacheUpdates      uint64
	TagFiltersToMetricIDsCacheMisses       uint64
	TagFiltersToMetricIDsCacheCollisions   uint64
...
func (db *indexDB) UpdateMetrics(m *IndexDBMetrics) {
	...
	cs := db.tagFiltersToMetricIDsCache.stats()
	...
	m.TagFiltersToMetricIDsCacheUpdates += cs.setCalls
	m.TagFiltersToMetricIDsCacheCollisions += cs.collisions
...
func (db *indexDB) saveDeletedMetricIDs(metricIDs *uint64set.Set) {
	...
	// Reset TagFilters -> metricIDs cache, since it may contain deleted TSIDs.
...
func (db *indexDB) searchMetricIDs(qt *querytracer.Tracer, tfss []*TagFilters, tr TimeRange, maxMetrics int, deadline uint64) (*uint64set.Set, error) {
	...
	metricIDs, ok := db.tagFiltersToMetricIDsCache.get(qt, tfKeyBuf.B)
	if ok {
	...
	db.tagFiltersToMetricIDsCache.set(qt, metricIDs, tfKeyBuf.B)
