...
// the given tr.
func (db *indexDB) SearchLabelNames(qt *querytracer.Tracer, accountID, projectID uint32, tfss []*TagFilters, tr TimeRange, maxLabelNames, maxMetrics int, deadline uint64) ([]string, error) {
...
func (db *indexDB) SearchTenants(qt *querytracer.Tracer, tr TimeRange, deadline uint64) ([]string, error) {
	...
	err := is.searchTenantsOnTimeRange(qt, tenants, tr)
	db.putIndexSearch(is)
	if err != nil {
		return nil, err
	}
	...
	qt.Printf("found %d tenants", len(tenantsList))
...
func (db *indexDB) SearchLabelValues(qt *querytracer.Tracer, accountID, projectID uint32, labelName string, tfss []*TagFilters, tr TimeRange, maxLabelValues, maxMetrics int, deadline uint64) ([]string, error) {
	...
	lvs, err := db.searchLabelValues(qt, accountID, projectID, labelName, tfss, tr, maxLabelValues, maxMetrics, deadline)
	qt.Donef("found %d label values", len(lvs))
	return lvs, err
}
...
func (db *indexDB) SearchGraphitePaths(qt *querytracer.Tracer, accountID, projectID uint32, tr TimeRange, qHead, qTail []byte, maxPaths int, deadline uint64) ([]string, error) {
...
func (db *indexDB) SearchMetricNames(qt *querytracer.Tracer, tfss []*TagFilters, tr TimeRange, maxMetrics int, deadline uint64) ([]string, error) {
	...
	accountID := tfss[0].accountID
	projectID := tfss[0].projectID
	if err = db.prefetchMetricNames(qt, accountID, projectID, metricIDs, deadline); err != nil {
		return nil, err
	}
...
func (db *indexDB) prefetchMetricNames(qt *querytracer.Tracer, accountID, projectID uint32, srcMetricIDs []uint64, deadline uint64) error {
...
func (is *indexSearch) hasDateMetricID(date, metricID uint64, accountID, projectID uint32) bool {
	if date == globalIndexDate {
		return is.hasMetricID(metricID, accountID, projectID)
	}
