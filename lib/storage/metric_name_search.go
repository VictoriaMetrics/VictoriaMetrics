...
func (s *metricNameSearch) search(dst []byte, metricID uint64, accountID, projectID uint32) ([]byte, bool) {
	if !s.useSparseCache {
		n := len(dst)
	...
	dst, found := s.idbCurr.searchMetricName(dst, metricID, accountID, projectID, s.useSparseCache)
	if found {
		if !s.useSparseCache {
	...
	dst, found = s.idbPrev.searchMetricName(dst, metricID, accountID, projectID, s.useSparseCache)
	if found {
		if !s.useSparseCache {
			s.storage.putMetricNameToCache(metricID, dst)
		}
