...
func loadFrom(loadPath string, maxSizeBytes uint64) (*Tracker, error) {
	mt := &Tracker{
		...
		store:          make(map[statKey]*statItem),
		metricNamesBuf: make([]byte, 0, metricNameBufSize),
		statItemBuf:    make([]statItem, 0, statItemBufSize),
		cachePath:      loadPath,
		getCurrentTs:   fasttime.UnixTimestamp,
	}
	mt.creationTs.Store(fasttime.UnixTimestamp())
	...
	if f != nil {
		...
		for {
			...
			metricName := bytesutil.ToUnsafeString(mt.allocateMetricNameLocked([]byte(r.MetricName)))
			key := statKey{
...
func (mt *Tracker) nextRecordLocked() *statItem {
	...
	siStoreNew := make([]statItem, 0, statItemBufSize)
	mt.statItemBuf = siStoreNew[:1]
	return &mt.statItemBuf[0]
}

func (mt *Tracker) allocateMetricNameLocked(metricGroup []byte) []byte {
	idx := len(mt.metricNamesBuf)
	n := len(metricGroup) + len(mt.metricNamesBuf)
	...
	mt.metricNamesBuf = append(mt.metricNamesBuf, metricGroup...)
	return mt.metricNamesBuf[idx:]
}
...
func (mt *Tracker) Reset(onReset func()) {
	...
	mt.metricNamesBuf = make([]byte, 0, 16*1024)
	mt.statItemBuf = make([]statItem, 0, 1024)
...
func (mt *Tracker) RegisterIngestRequest(accountID, projectID uint32, metricGroup []byte) {
	if mt == nil {
		return
	}
	...
	sk := statKey{accountID: accountID, projectID: projectID, metricName: bytesutil.ToUnsafeString(metricGroup)}
	mt.mu.RLock()
	_, ok := mt.store[sk]
	mt.mu.RUnlock()
	if ok {
	...
	si := mt.nextRecordLocked()
	sk.metricName = bytesutil.ToUnsafeString(mt.allocateMetricNameLocked(metricGroup))
	mt.store[sk] = si
	mt.mu.Unlock()

	mt.currentSizeBytes.Add(uint64(len(metricGroup)) + storeOverhead)
	mt.currentItemsCount.Add(1)
}

// RegisterQueryRequest tracks metric name at query request
func (mt *Tracker) RegisterQueryRequest(accountID, projectID uint32, metricGroup []byte) {
	if mt == nil {
		return
	}
	mt.mu.RLock()
	si, ok := mt.store[statKey{accountID: accountID, projectID: projectID, metricName: bytesutil.ToUnsafeString(metricGroup)}]
	mt.mu.RUnlock()
	if !ok {
		return
	}
