...
func (s *Storage) GetMetricNamesStats(_ *querytracer.Tracer, at *TenantToken, limit, le int, matchPattern string) MetricNamesStatsResponse {
	if at != nil {
		return s.metricsTracker.GetStatsForTenant(at.AccountID, at.ProjectID, limit, le, matchPattern)
	}
	r := s.metricsTracker.GetStats(limit, le, matchPattern)
	r.DeduplicateMergeRecords()
