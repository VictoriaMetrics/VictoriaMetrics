...
func BenchmarkFilterAdd(b *testing.B) {
	for _, maxItems := range []int32{1e3, 1e4, 1e5, 1e6, 1e7} {
...
func benchmarkFilterAdd(b *testing.B, maxItems int32) {
	b.ReportAllocs()
...
func benchmarkFilterHasHit(b *testing.B, maxItems int32) {
	b.ReportAllocs()
	b.RunParallel(func(pb *testing.PB) {
		f := newFilter(maxItems)
		h := uint64(0)
...
func benchmarkFilterHasMiss(b *testing.B, maxItems int32) {
	b.ReportAllocs()
	b.RunParallel(func(pb *testing.PB) {
		f := newFilter(maxItems)
		for pb.Next() {
