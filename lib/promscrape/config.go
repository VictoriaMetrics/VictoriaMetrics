...
type GlobalConfig struct {
	LabelsLimit          int                         `yaml:"labels_limit,omitempty"`
...
func getScrapeWorkConfig(sc *ScrapeConfig, baseDir string, globalCfg *GlobalConfig) (*scrapeWorkConfig, error) {
	...
	labelsLimit := sc.LabelsLimit
	if labelsLimit <= 0 {
		labelsLimit = globalCfg.LabelsLimit
		if labelsLimit < 0 {
			labelsLimit = defaultLabelsLimit
		}
	}
