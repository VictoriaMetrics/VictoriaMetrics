...
func testScrapeWorkScrapeInternalSuccess(t *testing.T, streamParse bool) {
	f := func(data string, cfg *ScrapeWork, dataExpected string) {
		...
		tsmGlobal.Register(&sw)
		if err := sw.scrapeInternal(timestamp, timestamp); err != nil {
			if !strings.Contains(err.Error(), "sample_limit") && !strings.Contains(err.Error(), "labels_limit") {
...
`),
	...
	`, &ScrapeWork{
		...
		scrape_timeout_seconds 42 123
	`)
	// Scrape failure because of the exceeded LabelsLimit
	f(`
                foo{bar="baz"} 34.44
                bar{a="b",c="d",e="f"} -3e4
        `, &ScrapeWork{
...
func TestWriteRequestCtx_TryAddRowNoRelabeling(t *testing.T) {
	f := func(row string, cfg *ScrapeWork, dataExpected string) {
		...
		wc.tryAddRow(cfg, r, r.Timestamp, false)
