...
func TestGetInstances(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			// Prepare a no-op discovery HTTP client. Actions of this client are monkey patched.
			client, _ := discoveryutils.NewClient("http://127.0.0.1", nil, nil, nil, &promauth.HTTPClientConfig{})
			cfg := &apiConfig{
				c: client,
			}

			// Patching `GetAPIResponse` method. The Patch will be reset for each test case.
			patch := gomonkey.ApplyMethod(client, "GetAPIResponse", func(_ *discoveryutils.Client, _ string) ([]byte, error) {
				var e error
				if tt.apiError {
