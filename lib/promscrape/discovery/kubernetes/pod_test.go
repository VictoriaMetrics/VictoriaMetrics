...
func TestParsePodListSuccess(t *testing.T) {
	f := func(response, contentType string, expectedLabels []*promutils.Labels) {
		t.Helper()
	...
	f(testPodsList, contentTypeJSON, []*promutils.Labels{
	...
	f(testPodsListIPv6Address, contentTypeJSON, []*promutils.Labels{
		promutils.NewLabelsFromMap(map[string]string{
	...
	f(testPodsListIPv6AddressNoPorts, contentTypeJSON, []*promutils.Labels{
		promutils.NewLabelsFromMap(map[string]string{
