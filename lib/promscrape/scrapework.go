...
func (sw *ScrapeWork) key() string {
	...
	key := fmt.Sprintf("JobNameOriginal=%s, ScrapeURL=%s, ScrapeInterval=%s, ScrapeTimeout=%s, HonorLabels=%v, "+
		...
		"ScrapeAlignInterval=%s, ScrapeOffset=%s, SeriesLimit=%d, LabelsLimit=%d, NoStaleMarkers=%v",
		...
		sw.ScrapeAlignInterval, sw.ScrapeOffset, sw.SeriesLimit, sw.LabelsLimit, sw.NoStaleMarkers)
	return key
}
...
var (
	...
	scrapesSkippedByLabelsLimit = metrics.NewCounter("vm_promscrape_scrapes_skipped_by_labels_limit_total")
...
func (sw *scrapeWork) processDataOneShot(scrapeTimestamp, realTimestamp int64, body []byte, scrapeDurationSeconds float64, err error) error {
	...
	dropScrape := !wc.tryAddRows(cfg, wc.rows.Rows, scrapeTimestamp, true) && cfg.LabelsLimit > 0
	if dropScrape {
		scrapesSkippedByLabelsLimit.Inc()
		err = fmt.Errorf("the response from %q exceeds labels_limit=%d; "+
			"either reduce the labels count for exceeding series or increase labels_limit", cfg.ScrapeURL, cfg.LabelsLimit)
	} else {
		samplesPostRelabeling = len(wc.writeRequest.Timeseries)
		dropScrape = cfg.SampleLimit > 0 && samplesPostRelabeling > cfg.SampleLimit
		if dropScrape {
...
func (sw *scrapeWork) processDataInStreamMode(scrapeTimestamp, realTimestamp int64, body *bytesutil.ByteBuffer, scrapeDurationSeconds float64) error {
	...
	err := stream.Parse(r, scrapeTimestamp, "", false, func(rows []parser.Row) error {
		...
		if !wc.tryAddRows(cfg, rows, scrapeTimestamp, true) && cfg.LabelsLimit > 0 {
			scrapesSkippedByLabelsLimit.Inc()
			return fmt.Errorf("the response from %q exceeds labels_limit=%d; "+
				"either reduce the labels count for exceeding series or increase labels_limit", cfg.ScrapeURL, cfg.LabelsLimit)
		}
...
func (sw *scrapeWork) sendStaleSeries(lastScrape, currScrape string, timestamp int64, addAutoSeries bool) {
	...
	if bodyString != "" {
		...
		err := stream.Parse(br, timestamp, "", false, func(rows []parser.Row) error {
			...
			wc.tryAddRows(sw.Config, rows, timestamp, true)
...
func (wc *writeRequestCtx) addAutoMetrics(sw *scrapeWork, am *autoMetrics, timestamp int64) {
	...
	if labelsLimit := sw.Config.LabelsLimit; labelsLimit > 0 {
		dst = appendRow(dst, "scrape_labels_limit", float64(labelsLimit), timestamp)
	}
	...
	wc.tryAddRows(sw.Config, dst, timestamp, false)
...
func (wc *writeRequestCtx) tryAddRows(cfg *ScrapeWork, rows []parser.Row, timestamp int64, needRelabel bool) bool {
	...
	// add rows
	for i := range rows {
		if !wc.tryAddRow(cfg, &rows[i], timestamp, needRelabel) {
			return false
		}
	}
	return true
...
func (wc *writeRequestCtx) tryAddRow(cfg *ScrapeWork, r *parser.Row, timestamp int64, needRelabel bool) bool {
	metric := r.Metric
	...
	if needRelabel && labelsLimit > 0 && len(wc.labels)-labelsLen > labelsLimit {
