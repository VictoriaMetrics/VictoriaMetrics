...
func (sw *scrapeWork) processDataOneShot(scrapeTimestamp, realTimestamp int64, body []byte, scrapeDurationSeconds float64, err error) error {
	...
	dropScrape := !wc.tryAddRows(cfg, wc.rows.Rows, scrapeTimestamp, true) && cfg.LabelLimit > 0
	if dropScrape {
		scrapesSkippedByLabelLimit.Inc()
		err = fmt.Errorf("the response from %q exceeds label_limit=%d; "+
			"either reduce the labels count for exceeding series or increase label_limit", cfg.ScrapeURL, cfg.LabelLimit)
	} else {
...
func (sw *scrapeWork) processDataInStreamMode(scrapeTimestamp, realTimestamp int64, body *bytesutil.ByteBuffer, scrapeDurationSeconds float64) error {
	...
	err := stream.Parse(r, scrapeTimestamp, "", false, func(rows []parser.Row) error {
		...
		if !wc.tryAddRows(cfg, rows, scrapeTimestamp, true) && cfg.LabelLimit > 0 {
			scrapesSkippedByLabelLimit.Inc()
