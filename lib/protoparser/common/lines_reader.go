...
// The maximum size of a single line returned by ReadBlock.
const maxLineSize = 256 * 1024

// Default size in bytes of a single block returned by ReadBlock.
const defaultBlockSize = 64 * 1024

// ReadBlock reads a block of lines delimited by '\n' from tailBuf and r into dstBuf.
//
...
func ReadBlock(r io.Reader, dstBuf, tailBuf []byte, isBinary bool) ([]byte, []byte, error) {
	return ReadBlockExt(r, dstBuf, tailBuf, maxLineSize, isBinary)
}

// ReadBlockExt reads a block of lines delimited by '\n' from tailBuf and r into dstBuf.
//
// Trailing chars after the last newline are put into tailBuf.
//
// Returns (dstBuf, tailBuf).
...
//
// It is expected that read timeout on r exceeds 1 second.
func ReadBlockExt(r io.Reader, dstBuf, tailBuf []byte, maxLineLen int, isBinary bool) ([]byte, []byte, error) {
	startTime := time.Now()
