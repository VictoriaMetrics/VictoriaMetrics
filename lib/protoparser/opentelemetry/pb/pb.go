...
var (
	rowsDroppedNegativeBuckets    = metrics.NewCounter(`vm_protoparser_rows_dropped_total{type="opentelemetry",reason="negative_exponential_histogram_buckets"}`)
	negativeHistogramBucketLogger = logger.WithThrottler("otlp_exp_histogram_negative_buckets", 5*time.Second)
)

func (ehctx *exponentialHistogramDataPointContext) pushSamples(dctx *decoderContext) {
	// VictoriaMetrics native histograms do not support negative buckets, so drop data points in the negative range
	if len(ehctx.negative.bucketCounts) > 0 {
		rowsDroppedNegativeBuckets.Inc()
		negativeHistogramBucketLogger.Warnf("opentelemetry: dropping exponential histogram data point with negative buckets. " +
			"See https://docs.victoriametrics.com/victoriametrics/integrations/opentelemetry/#exponential-histograms.")
		return
	}
