...
func TestParse(t *testing.T) {
	...
	f := func(s string, rowsExpected []prometheus.Row) {
		...
		err := Parse(bb, defaultTimestamp, "", true, func(rows []prometheus.Row, _ []prometheus.Metadata) error {
			lock.Lock()
		...
		result = nil
		doneCh = make(chan struct{})
		err = Parse(bb, defaultTimestamp, "gzip", false, func(rows []prometheus.Row, _ []prometheus.Metadata) error {
			lock.Lock()
			result = appendRowCopies(result, rows)
			if len(result) == len(rowsExpected) {
				close(doneCh)
