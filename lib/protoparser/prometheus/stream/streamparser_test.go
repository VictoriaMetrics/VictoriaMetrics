...
func TestParse(t *testing.T) {
	...
	f := func(s string, rowsExpected []prometheus.Row) {
		...
		err := Parse(bb, prometheus.TextHeader, defaultTimestamp, false, true, func(rows []prometheus.Row) error {
			lock.Lock()
		...
		result = nil
		doneCh = make(chan struct{})
		err = Parse(bb, prometheus.TextHeader, defaultTimestamp, true, false, func(rows []prometheus.Row) error {
			lock.Lock()
			result = appendRowCopies(result, rows)
			if len(result) == len(rowsExpected) {
				close(doneCh)
