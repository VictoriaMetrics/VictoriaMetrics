...
func TestParse(t *testing.T) {
	testMode = true
	f := func(data string, rowsExpected []influx.Row, isStreamMode bool, precesion string) {
		...
		errsCond := err == nil
		if isStreamMode {
			errsCond = err.Error() == "error when processing imported data: test"
		}
		if !errsCond {
...
foo4,location=us-midwest4 temperature=84 1727879909390000000
foo5,location=us-midwest5 temperature=85 1727879909390000000
`, []influx.Row{
		{
			Measurement: "foo1",
			...
			Measurement: "foo4",
			Tags:        []influx.Tag{{Key: "location", Value: "us-midwest4"}},
			Fields:      []influx.Field{{Key: "temperature", Value: 84}},
			Timestamp:   1727879909390,
		}, {
			Measurement: "foo5",
			Tags:        []influx.Tag{{Key: "location", Value: "us-midwest5"}},
			Fields:      []influx.Field{{Key: "temperature", Value: 85}},
			Timestamp:   1727879909390,
		}}, false, "ns")
