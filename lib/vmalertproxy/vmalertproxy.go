...
func Init(proxyURLs, proxyNames []string) {
	...
	for i, proxyURL := range proxyURLs {
		...
		// set the name to ``vmalert_proxy_N`` by default, do not use the raw URL as a name, since it may contain sensitive data (e.g. auth credentials).
		...
		b.rp = nethttputil.NewSingleHostReverseProxy(pu)
		b.rp.ErrorHandler = func(w http.ResponseWriter, _ *http.Request, err error) {
			b.errors.Inc()
...
//
// The returned names are used as values for the SourceLabel meta-label and for the `vmalert_source` query arg.
...
//
// Unavailable vmalerts don't fail the whole request, but report via the `warnings` field of the response.
//
// Pass `vmalert_source` query arg for proxying the request to the given vmalert only.
func HandleRequest(w http.ResponseWriter, r *http.Request, path string) {
	...
	// The path cannot be merged across multiple vmalerts is proxied to the first configured vmalert.
	bs[0].proxyRequest(w, r, path)
}
