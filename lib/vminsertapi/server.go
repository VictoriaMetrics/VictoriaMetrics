...
func NewVMInsertServer(addr string, connectionTimeout time.Duration, listenerName string, api API) (*VMInsertServer, error) {
	...
	s := &VMInsertServer{
		...
		vminsertConns:        metrics.GetOrCreateCounter(`vm_vminsert_conns` + labels),
		vminsertConnErrors:   metrics.GetOrCreateCounter(`vm_vminsert_conn_errors_total` + labels),
		vminsertMetricsRead:  metrics.GetOrCreateCounter(`vm_vminsert_metrics_read_total` + labels),
		vminsertMetadataRead: metrics.GetOrCreateCounter(`vm_vminsert_metadata_read_total` + labels),
	}
...
func (s *VMInsertServer) processWriteMetadata(ctx *vminsertRequestCtx) error {
	return stream.ParseMR(ctx.bc, func(mrs []metricsmetadata.Row) error {
		s.vminsertMetadataRead.Add(len(mrs))
		return s.api.WriteMetadata(mrs)
	}, s.api.IsReadOnly)
}
