...
func (t *Tracer) AddStats(key string, value float64) {
	if t == nil {
		return
	}
	if !t.doneTime.IsZero() {
		panic(fmt.Errorf("BUG: AddStats() cannot be called after Done(%q) call", t.message))
	}
	t.stats[key] += value
}
...
func (t *Tracer) toSpanInternal(prevTime time.Time) (*span, time.Time) {
	...
	for _, child := range t.children {
		...
		for k, v := range sChild.Stats {
			stats[k] += v
		}
		// reset the stats, so only root span
		// will have the accumulated stats
		sChild.Stats = nil
