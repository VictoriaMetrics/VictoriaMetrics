...
func (p *sharedPool) reset() {
	...
	exemplarLabelsPool := p.exemplarLabels
	for i := range exemplarLabelsPool {
		exemplarLabelsPool[i] = Label{}
	}
	p.exemplarLabels = exemplarLabelsPool[:0]
	...
	exemplarsPool := p.exemplars
	for i := range exemplarsPool {
		exemplarsPool[i] = Exemplar{}
	}
	p.exemplars = exemplarsPool[:0]
...
func (ts *TimeSeries) unmarshalProtobuf(src []byte, p *sharedPool) error {
	...
	exemplarsPoolLen := len(p.exemplars)
	...
	ts.Exemplars = p.exemplars[exemplarsPoolLen:]
...
func (exemplar *Exemplar) unmarshalProtobuf(src []byte, labelsPool []Label) ([]Label, error) {
	...
	for len(src) > 0 {
		...
		case 3:
			ts, ok := fc.Int64()
			if !ok {
				return labelsPool, fmt.Errorf("cannot read exemplar timestamp data")
			}
