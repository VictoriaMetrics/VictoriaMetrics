...
type API interface {
	...
	TagValueSuffixes(qt *querytracer.Tracer, tr storage.TimeRange, tagKey, tagValuePrefix string, delimiter byte, maxSuffixes int, deadline uint64) ([]string, error)
	...
	SeriesCount(qt *querytracer.Tracer, deadline uint64) (uint64, error)
	...
	GetMetricNamesUsageStats(qt *querytracer.Tracer, limit, le int, matchPattern string, deadline uint64) (storage.MetricNamesStatsResponse, error)
