...
func NewServer(addr string, accountID, projectID uint32, api API, limits Limits, disableResponseCompression bool) (*Server, error) {
...
func (ctx *vmselectRequestCtx) sameTenant(accountID, projectID uint32) bool {
	if len(ctx.sq.TenantTokens) == 0 {
		return true
	}
	return ctx.sq.TenantTokens[0].AccountID == accountID && ctx.sq.TenantTokens[0].ProjectID == projectID
}

func (ctx *vmselectRequestCtx) cancel() error {
	if err := ctx.writeUint64(0); err != nil {
		return fmt.Errorf("cannot cancel request: %w", err)
	}
	if err := ctx.writeUint64(0); err != nil {
		return fmt.Errorf("cannot cancel request: %w", err)
	}
...
func (ctx *vmselectRequestCtx) skipBytes(n int64) error {
	if _, err := io.CopyN(io.Discard, ctx.bc, n); err != nil {
...
func (s *Server) processDeleteSeries(ctx *vmselectRequestCtx) error {
	...
	if !ctx.sameTenant(s.accountID, s.projectID) {
...
func (s *Server) processTagValueSuffixes(ctx *vmselectRequestCtx) error {
	...
	if accountID != s.accountID || projectID != s.projectID {
		if err := ctx.cancel(); err != nil {
			return err
		}
		return nil
	...
	// Execute the request
	suffixes, err := s.api.TagValueSuffixes(ctx.qt, tr, tagKey, tagValuePrefix, delimiter, maxSuffixes, ctx.deadline)
	if err != nil {
		return ctx.writeErrorMessage(err)
	}
...
func (s *Server) processSeriesCount(ctx *vmselectRequestCtx) error {
	...
	// Execute the request
	n, err := s.api.SeriesCount(ctx.qt, ctx.deadline)
	if err != nil {
		return ctx.writeErrorMessage(err)
	}
...
func (s *Server) processTenants(ctx *vmselectRequestCtx) error {
	...
	// skip reading 16 bytes of time range
	if err := ctx.skipBytes(16); err != nil {
		return err
	}
	...
	tenant := fmt.Sprintf("%d:%d", s.accountID, s.projectID)
...
func (s *Server) processSearchMetricNames(ctx *vmselectRequestCtx) error {
	...
	var buf []byte
	buf = encoding.MarshalUint32(buf, s.accountID)
	buf = encoding.MarshalUint32(buf, s.projectID)
	for i, metricName := range metricNames {
		ctx.dataBuf = append(ctx.dataBuf[:0], buf...)
		ctx.dataBuf = append(ctx.dataBuf, metricName...)
...
func (s *Server) processSearch(ctx *vmselectRequestCtx) error {
	...
	for bi.NextBlock(&ctx.mb) {
		...
		ctx.dataBuf = ctx.mb.MarshalWithTenant(ctx.dataBuf[:0], s.accountID, s.projectID)
		if err := ctx.writeDataBufBytes(); err != nil {
...
func (s *Server) processMetricNamesUsageStats(ctx *vmselectRequestCtx) error {
	...
	result, err := s.api.GetMetricNamesUsageStats(ctx.qt, limit, int(le), matchPattern, ctx.deadline)
	if err != nil {
		return ctx.writeErrorMessage(err)
	}
