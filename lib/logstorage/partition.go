...
var (
	vlStreamIDCacheHits   = metrics.NewCounter(`vl_cache_hits_total{type="stream_id"}`)
	vlStreamIDCacheMisses = metrics.NewCounter(`vl_cache_misses_total{type="stream_id"}`)
)
...
func (pt *partition) mustAddRows(lr *LogRows) {
	...
	vlStreamIDCacheMisses.Add(int(cacheMisses))
	vlStreamIDCacheHits.Add(int(cacheHits))
