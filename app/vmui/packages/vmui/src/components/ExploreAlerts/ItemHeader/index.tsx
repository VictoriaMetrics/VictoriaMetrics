...
import { groupSourceParam } from "../helpers";
...
interface ItemHeaderControlsProps {
  ...
  type?: string;
  groupId: string;
  // source is the name of the vmalert owning the entity. See getVMAlertSource().
  source: string;
...
const ItemHeader: FC<ItemHeaderControlsProps> = ({ name, id, groupId, source, entity, type, states, onClose, classes }) => {
  const { isMobile } = useDeviceDetect();
  const { serverUrl } = useAppState();
  const navigate = useNavigate();
  const copyToClipboard = useCopyToClipboard();

  // Entity IDs are generated by every vmalert independently, so the link must carry
  // the vmalert owning the entity. Otherwise vmselect rejects the request for the entity
  // details, since it cannot tell which of the -vmalert.proxyURL owns the ID.
  const sourceParam = groupSourceParam(source);

  const openGroupLink = () => {
    navigate({
      pathname: "/rules",
      search:   `group_id=${groupId}${sourceParam}`,
    });
  };
  ...
  const openItemLink = () => {
    navigate({
      pathname: "/rules",
      search:   `group_id=${groupId}&${entity}_id=${id}${sourceParam}`,
    });
  };
