...
describe("fetchRawQueryCSVExport", () => {
  it("requests all label columns before exporting CSV data", async () => {
    ...
    expect(fetchMock.mock.calls[1][0]).toBe("http://localhost:8428/api/v1/export/csv?start=1710000000&end=1710000300&format=__name__%2Cinstance%2Cjob%2C__value__%2C__timestamp__%3Aunix_ms&match%5B%5D=up");
    expect(result).toBe("name,instance,job,value,timestamp\nup,localhost:9100,node_exporter,1,1710000000000");
  });
});
