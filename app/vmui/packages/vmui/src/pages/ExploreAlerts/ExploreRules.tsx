...
const defaultRuleTypesStr = getQueryStringValue("ruleTypes", "") as string;
...
const ExploreRules: FC<ExploreRulesProps> = ({ ruleTypeFilter }) => {
  const { groups, isLoading: loadingGroups, error: errorGroups } = useFetchGroups({ ruleTypeFilter });

  const [expandGroups, setExpandGroups] = useState<Set<string>>(new Set<string>());
  const [expandRules, setExpandRules] = useState<Set<string>>(new Set<string>());
  ...
  groups.forEach((group) => {
    ...
    group.rules.forEach((rule) => {
      ...
      if (ruleTypes?.length && !ruleTypes.includes(ruleType)) return;
      ...
      if (searchInput && !searchRegex.test(rule.name) && !searchRegex.test(group.name)) return;

      filteredRules.push(rule);
      if (state !== "no match" && state !== "unhealthy" && state !== "firing") return;
  ...
  if (ruleTypeFilter === "alert") {
    noRuleFound = "No alerting rules found!";
  } else if (ruleTypeFilter === "record") {
