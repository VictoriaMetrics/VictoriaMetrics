...
var (
	otsdbSeriesTotal     = vmetrics.NewCounter(`vmctl_migration_series_total{mode="opentsdb"}`)
	otsdbSeriesProcessed = vmetrics.NewCounter(`vmctl_migration_series_processed{mode="opentsdb"}`)
	otsdbErrorsTotal     = vmetrics.NewCounter(`vmctl_migration_errors_total{mode="opentsdb"}`)
)
