...
var (
	migrationMetricsTotal     = metrics.NewCounter(`vmctl_migration_metrics_total{mode="vm-native"}`)
	migrationMetricsProcessed = metrics.NewCounter(`vmctl_migration_metrics_processed{mode="vm-native"}`)

	migrationRequestsPlanned   = metrics.NewCounter(`vmctl_migration_requests_planned{mode="vm-native"}`)
	migrationRequestsCompleted = metrics.NewCounter(`vmctl_migration_requests_completed{mode="vm-native"}`)

	migrationErrorsTotal = metrics.NewCounter(`vmctl_migration_errors_total{mode="vm-native"}`)

	migrationTenantsTotal     = metrics.NewCounter(`vmctl_migration_tenants_total{mode="vm-native"}`)
	migrationTenantsProcessed = metrics.NewCounter(`vmctl_migration_tenants_processed{mode="vm-native"}`)

	migrationBytesTransferredTotal = metrics.NewCounter(`vmctl_migration_bytes_transferred_total{mode="vm-native"}`)
)
