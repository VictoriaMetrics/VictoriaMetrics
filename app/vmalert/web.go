...
func (rh *requestHandler) handler(w http.ResponseWriter, r *http.Request) bool {
	...
	case "/vmalert/groups":
		...
		data := rh.groups(rf)
		WriteListGroups(w, r, data, rf.condition)
		return true
...
func newRulesFilter(r *http.Request) (*rulesFilter, error) {
	...
	cond := query.Get("cond")
	if len(cond) > 0 {
		if cond == "nomatch" || cond == "unhealthy" {
			rf.condition = cond
		} else {
			return nil, errResponse(fmt.Errorf(`invalid parameter "cond": not supported value %q`, cond), http.StatusBadRequest)
		}
	}
...
func (rh *requestHandler) groups(rf *rulesFilter) []*apiGroup {
	...
	for _, group := range rh.m.groups {
		...
		for _, rule := range g.Rules {
			...
			if (rule.LastError == "" && rf.condition == "unhealthy") || (!isNoMatch(rule) && rf.condition == "nomatch") {
				continue
			}
			if rf.excludeAlerts {
				rule.Alerts = nil
