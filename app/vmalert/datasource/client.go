...
func toDatasourceType(s string) datasourceType {
	...
	default:
		logger.Panicf("BUG: unexpected datasource type %q", s)
	}
	return ""
}
...
// WARN: when adding a new field, remember to check if CloneFields() method needs to be updated.
type Client struct {
	c                *http.Client
...
// CloneFields returns a new client with inheritable fields from original client.
func (s *Client) CloneFields() *Client {
	ns := &Client{
		c:                s.c,
		authCfg:          s.authCfg,
...
func (s *Client) BuildWithParams(params QuerierParams) Querier {
	return s.CloneFields().ApplyParams(params)
}
