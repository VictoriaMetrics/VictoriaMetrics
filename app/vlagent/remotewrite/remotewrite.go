...
var (
	remoteWriteURLs = flagutil.NewArrayString("remoteWrite.url", "Remote storage URL to write data to. It must support VictoriaLogs native rotocol. "+
	...
	tmpDataPath = flag.String("remoteWrite.tmpDataPath", "lvagent-native-write", "Path to directory for storing pending data, which isn't sent to the configured -remoteWrite.url . "+
...
// the maximum size of a single data block sent to remote storage.
...
func newRemoteWriteCtx(argIdx int, remoteWriteURL *url.URL, maxInmemoryBlocks int, sanitizedURL string) *remoteWriteCtx {
	...
	rwctx := &remoteWriteCtx{
		...
		rowsDroppedOnPushFailure: metrics.GetOrCreateCounter(fmt.Sprintf(`vlagent_remotewrite_samples_dropped_total{path=%q,url=%q}`, queuePath, sanitizedURL)),
	}
