...
func TestAllowRerouting(t *testing.T) {
	...
	*disableRerouting = false
	...
	newStorage := func(avgSendDuration float64, ready bool) *storageNode {
		sn := &storageNode{
			avgSendDuration: ewma.NewMovingAverage(),
	...
	// rerouting triggered on the slowest node if cluster avg saturation less than 0.5
	f([]*storageNode{
	...
	*disableRerouting = false
	...
	// rerouting not triggered because cluster avg saturation more than 0.4
	f([]*storageNode{
		newStorage(0.801, true),
