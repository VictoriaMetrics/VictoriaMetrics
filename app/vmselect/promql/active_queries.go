...
func writeActiveQueries(w http.ResponseWriter, aqes []activeQueryEntry) {
	...
	for i, aqe := range aqes {
		...
		fmt.Fprintf(w, `{"duration":"%.3fs","id":"%016X","remote_addr":%s,"account_id":"%d","project_id":"%d","query":%q,"start":%d,"end":%d,"step":%d, "is_multitenant":%v}`,
			d.Seconds(), aqe.qid, aqe.quotedRemoteAddr, aqe.accountID, aqe.projectID, aqe.q, aqe.start, aqe.end, aqe.step, aqe.isMultitenant)
		if i+1 < len(aqes) {
			fmt.Fprintf(w, `,`)
		}
	}
