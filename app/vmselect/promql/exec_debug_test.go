...
func TestSimulatedExec(t *testing.T) {
	...
	t.Run(`simple_metric_exact_match`, func(t *testing.T) {
		...
		ec.SimulatedSamples = []*storage.SimulatedSample{mn.build()}
...
func TestSumOverTime(t *testing.T) {
	...
	t.Run(`basic_sum_over_time`, func(t *testing.T) {
		...
		ec.SimulatedSamples = []*storage.SimulatedSample{metric.build()}
...
type metricBuilder storage.SimulatedSample
...
func (b *metricBuilder) build() *storage.SimulatedSample {
	return (*storage.SimulatedSample)(b)
}
...
func (b metricBuilders) build() []*storage.SimulatedSample {
	ss := make([]*storage.SimulatedSample, len(b))
	for i := range b {
		ss[i] = b[i].build()
	}
	return ss
...
func (b metricBuilders) toResults() []netstorage.Result {
	results := make([]netstorage.Result, len(b))
	for i := range b {
		results[i] = b[i].toResult()
	}
	return results
}
