...
func (api *vmstorageAPI) GetMetricNamesUsageStats(qt *querytracer.Tracer, at *storage.TenantToken, le, limit int, matchPattern string, deadline uint64) (storage.MetricNamesStatsResponse, error) {
	dl := searchutils.DeadlineFromTimestamp(deadline)
	return netstorage.GetMetricNamesStats(qt, at, le, limit, matchPattern, dl)
}
