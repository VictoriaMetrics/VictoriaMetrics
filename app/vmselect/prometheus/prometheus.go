...
var (
	...
	maxTSDBStatusTopNSeries = flag.Int("search.maxTSDBStatusTopNSeries", 1000, "The maximum value of `topN` argument that can be passed to /api/v1/status/tsdb API. This option allows limiting memory usage. See https://docs.victoriametrics.com/victoriametrics/single-node-version/#tsdb-stats")
...
func ExportCSVHandler(startTime time.Time, w http.ResponseWriter, r *http.Request) error {
	...
	format := r.FormValue("format")
	if len(format) == 0 {
		return fmt.Errorf("missing `format` arg; see https://docs.victoriametrics.com/victoriametrics/single-node-version/#how-to-export-csv-data")
	}
