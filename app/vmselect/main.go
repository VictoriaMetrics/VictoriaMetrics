...
var (
	...
	metricNamesStatsResetAuthKey = flagutil.NewPassword("resetMetricsStatsAuthKey", "authKey for reseting metric names usage cache via /admin/api/v1/status/metric_names_stats/reset. It overrides -httpAuth.*")
...
func RequestHandler(w http.ResponseWriter, r *http.Request) bool {
	...
	case "/api/v1/status/metric_names_stats":
		...
		return true
		// use the same route prefix as vmselect cluster version
	case "/admin/api/v1/status/metric_names_stats/reset":
...
var (
	...
	metricNamesStatsResetRequests = metrics.NewCounter(`vm_http_requests_total{path="/admin/api/v1/status/metric_names_stats/reset"}`)
	metricNamesStatsResetErrors   = metrics.NewCounter(`vm_http_request_errors_total{path="/api/v1/admin/tsdb/metric_names_stats/reset"}`)
)
