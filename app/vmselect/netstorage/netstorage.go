...
func ExportBlocks(qt *querytracer.Tracer, sq *storage.SearchQuery, deadline searchutils.Deadline,
	...
	processBlock := func(mb *storage.MetricBlock, workerID uint) error {
		...
		if sq.IsMultiTenant {
			mn.AddTagBytes([]byte("vm_account_id"), strconv.AppendUint(nil, uint64(mn.AccountID), 10))
			mn.AddTagBytes([]byte("vm_project_id"), strconv.AppendUint(nil, uint64(mn.ProjectID), 10))
		}
