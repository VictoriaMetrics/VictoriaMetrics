...
// GetMetricNamesStats returns statistic for timeseries's metric names usage.
func GetMetricNamesStats(qt *querytracer.Tracer, limit, le int, matchPattern string) (storage.MetricNamesStatsResponse, error) {
	qt = qt.NewChild("get metric names usage statistics with limit: %d, less equal or equal to: %d, match pattern=%q", limit, le, matchPattern)
	defer qt.Done()
