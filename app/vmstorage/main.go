...
var (
	...
	tsidCacheKeyMode = flag.String("storage.tsidCacheKeyMode", "metricName", "EXPERIMENTAL: controls the key format for the "+
		...
		"fixed-size hash with an additional verifier, which can reduce cache memory usage for series with many or long labels. "+
		"See https://docs.victoriametrics.com/victoriametrics/single-server-victoriametrics/#cache-tuning")
...
func (vms *VMStorage) writeStorageMetrics(w io.Writer) {
	...
	// Counts fingerprint cache entries rejected because the stored verifier
	// didn't match. It is only updated when -storage.tsidCacheKeyMode=fingerprint.
	metrics.WriteCounterUint64(w, `vm_cache_verifier_mismatches_total{type="storage/tsid"}`, m.TSIDCacheVerifierMisses)
