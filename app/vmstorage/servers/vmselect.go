...
var (
	...
	defaultAccountID = flag.Int("accountID", 0, "account ID to allow to return data for")
	defaultProjectID = flag.Int("projectID", 0, "project ID to allow to return data for")
)
...
func GetAccountID() uint32 {
	return uint32(*defaultAccountID)
}

func GetProjectID() uint32 {
	return uint32(*defaultProjectID)
}
...
func NewVMSelectServer(addr string, s *storage.Storage) (*vmselectapi.Server, error) {
	...
	return vmselectapi.NewServer(addr, GetAccountID(), GetProjectID(), api, limits, *disableRPCCompression)
}
...
func (api *vmstorageAPI) TagValueSuffixes(qt *querytracer.Tracer, tr storage.TimeRange, tagKey, tagValuePrefix string, delimiter byte,
...
func (api *vmstorageAPI) SeriesCount(_ *querytracer.Tracer, deadline uint64) (uint64, error) {
...
func (api *vmstorageAPI) GetMetricNamesUsageStats(qt *querytracer.Tracer, limit, le int, matchPattern string, _ uint64) (storage.MetricNamesStatsResponse, error) {
